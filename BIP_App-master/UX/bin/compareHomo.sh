# 21/11/2007 par EVI : mise a jour de l'adresse du serveur d'HOMO (nouvelle adresse: 192.129.5.234 a la place de 192.128.128.234)
#02/05/2008 par EVI: mise a jour de l'adresse du serveur Homo suite a migration oracle 10 new:h-ora10-bip.dns30.socgen old:192.129.5.234 
#1) lister les fichiers versionnes


# fichier où sont listés les batch SQL à compiler
PROJET_HOME=/applis/bipd/bipdadm
...
# Pour tous les fichiers Batch (SQL) se trouvant sur l'homologation

#HOMO
REMOTE_HOST_BATCH_HOMO=h-ora10-bip.dns30.socgen
REMOTE_USER_BATCH_HOMO=hbipbftp
REMOTE_PWD_BATCH_HOMO=1TmdJYpz
REMOTE_HOME_BATCH_HOMO=
#REMOTE_HOME_BATCH2LOAD_HOMO=/datas/bippr/applis
REMOTE_HOME_BATCH2LOAD_HOMO=/batch/bipf/applis
REMOTE_HOME_BATCH3LOAD_HOMO=/applis/livra


# Pour tous les fichiers intranet (report,shell,tableau Excel) 

#HOMO
REMOTE_HOST_INTRA_HOMO=192.129.5.167
REMOTE_USER_INTRA_HOMO=ftpbip9
REMOTE_PWD_INTRA_HOMO=ftpbip9
REMOTE_HOME_INTRA_HOMO=/bip_wl9/data/reports/
REMOTE_HOME_INTRA2_HOMO=/bip_wl9/data/tableau/
...
REMOTE_DEST_LIVRA=/BIP/HOMO_$(date +"%Y%m%d")_reports
REMOTE_DEST_LIVRA2=/BIP/HOMO_$(date +"%Y%m%d")_tableau
REMOTE_DEST_LIVRA3=/BIP/HOMO

DIFF_FILE_BATCH=/applis/bipd/bipdadm/diff_batch_HOMO.lst
DIFF_FILE_INTRA=/applis/bipd/bipdadm/diff_intra_HOMO.lst
DIFF_FILE_INTRA2=/applis/bipd/bipdadm/diff_intra_e_HOMO.lst
DIFF_NOTEXIST_FILE_BATCH=/applis/bipd/bipdadm/diff_notexist_batch_HOMO.lst
DIFF_NOTEXIST_FILE_INTRA=/applis/bipd/bipdadm/diff_notexist_intra_HOMO.lst
DIFF_NOTEXIST_FILE_INTRA2=/applis/bipd/bipdadm/diff_notexist_intra_e_HOMO.lst
DIFF_LOG=/applis/bipd/bipdadm/diff_HOMO.log
SCRIPT_FTP=/applis/bipd/bipdadm/script_ftp_HOMO.sh
LISTE=/applis/bipd/bipdadm/liste_HOMO.lst
LIVR_FILE=/applis/bipd/bipdadm/livraison_HOMO.sh
LIVR_FILE_HOMO=/applis/bipd/bipdadm/livraison_HOMOLOGATION.sh
REP_DIFF=/applis/bipd/bipdadm/diff_work_HOMO
...
#********************************************************************************************************
# Etape 4 on récupère les fichiers du serveur de Homologation afin de les comparer ensuite à ceux du serveur de DEV

echo "ftp -vn <<! > /dev/null" > $SCRIPT_FTP
echo "open $REMOTE_HOST_BATCH_HOMO" >> $SCRIPT_FTP
echo "user $REMOTE_USER_BATCH_HOMO $REMOTE_PWD_BATCH_HOMO" >> $SCRIPT_FTP
echo "ascii" >> $SCRIPT_FTP
echo "cd $REMOTE_HOME_BATCH_HOMO" >> $SCRIPT_FTP
echo "dir" >> $SCRIPT_FTP
for fichier in $(cat $LISTE)
do
        echo "get $fichier $REP_DIFF/$fichier" >> $SCRIPT_FTP
done
echo "!" >> $SCRIPT_FTP
...
for fichier in $(	cat $LISTE | \
					...
					grep -v '^lost+found/' | \
					grep -v 'export_HOMO.sh' | \
					grep -v 'menu.csv')
do
        if [ ! -a $REP_DIFF/$fichier ]
        then
                echo "$fichier n'existe pas sur $REMOTE_HOST_BATCH_HOMO"
                echo "$fichier n'existe pas sur $REMOTE_HOST_BATCH_HOMO" >> $DIFF_LOG
...
echo "@$REMOTE_HOME_BATCH2LOAD_HOMO/$MESSAGE_FILE" >> $PROJET_HOME/$BATCH2COMPIL_FILE
echo " " >> $PROJET_HOME/$BATCH2COMPIL_FILE



#***************************************************************************************************************************
# Etape 7 a- création du fichier livraison_HOMO.sh qui permettra de charger les fichiers (*.sql,*.sh,*.rdf,*.xls etc) 
#            à partir de l'environnement de développement vers l'environnement de livraison 
#         b- création du fichier livraison_HOMOLOGATION.sh qui permettra de charger les fichiers (*.sql,*.sh,*.rdf,*.xls etc) 
#            à partir de l'environnement de livraison vers l'environnement de Homologation
...
echo "ftp -vn <<! > /dev/null" > $LIVR_FILE_HOMO
echo "open $REMOTE_HOST_BATCH_HOMO" >> $LIVR_FILE_HOMO
echo "user USER_FTP PWD_FTP" >> $LIVR_FILE_HOMO
echo "ascii" >> $LIVR_FILE_HOMO



#***************************************************************************************************************************
#Etape 8 remplir les fichiers livraison_homo.sh et livraison_homologation.sh
...
then
	...
	do
              ...
              echo "rename $REMOTE_HOME_BATCH2LOAD_HOMO/$fichier $REMOTE_HOME_BATCH2LOAD_HOMO/$fichier$(date +"%d%m%Y")" >> $LIVR_FILE_HOMO
              echo "put $REMOTE_HOME_BATCH3LOAD_HOMO$REMOTE_DEST_LIVRA3/$fichier $REMOTE_HOME_BATCH2LOAD_HOMO/$fichier" >> $LIVR_FILE_HOMO

	        case $fichier in
	        	*.sh) echo "quote site chmod +x $REMOTE_HOME_BATCH_HOMO/$fichier" >> $LIVR_FILE
                        echo "quote site chmod +x $REMOTE_DEST_LIVRA3/$fichier" >> $LIVR_FILE_HOMO;;
				# on crée un fichier de lancement automatique des batch
				*.sql) echo "@@$REMOTE_HOME_BATCH2LOAD_HOMO/$fichier" >> $PROJET_HOME/$BATCH2COMPIL_FILE;;
...
echo "quote site chmod +x $REMOTE_HOME_BATCH_HOMO/$COMPIL_BATCH" >> $LIVR_FILE
...
#On récupére les fichiers d'homologation reports

cd $REP_DIFF/intranet/reports
echo "ftp -vn <<! > /dev/null" > $SCRIPT_FTP
echo "open $REMOTE_HOST_INTRA_HOMO" >> $SCRIPT_FTP
echo "user $REMOTE_USER_INTRA_HOMO $REMOTE_PWD_INTRA_HOMO" >> $SCRIPT_FTP
echo "binary" >> $SCRIPT_FTP
echo "cd $REMOTE_HOME_INTRA_HOMO" >> $SCRIPT_FTP
...
do
    ...
    then
            echo "$fichier_long n'existe pas sur $REMOTE_HOST_INTRA_HOMO"
            echo "$fichier_long n'existe pas sur $REMOTE_HOST_INTRA_HOMO" >> $DIFF_LOG
...
#On récupére les fichiers de l'homologation Tableau Excel
...
echo "user $REMOTE_USER_INTRA_HOMO $REMOTE_PWD_INTRA_HOMO" >> $SCRIPT_FTP
echo "binary" >> $SCRIPT_FTP
echo "cd $REMOTE_HOME_INTRA2_HOMO" >> $SCRIPT_FTP
...
#echo "put $APP_HOME/compil/$COMP_INV_OBJ $REMOTE_DEST_LIVRA3/bin/$COMP_INV_OBJ" >> $LIVR_FILE
#echo "put $APP_HOME/$COMPIL_BATCH $REMOTE_DEST_LIVRA3/$COMPIL_BATCH" >> $LIVR_FILE
#echo "put $LIVR_FILE_HOMO $REMOTE_DEST_LIVRA3/bin/livraison_HOMOLOGATION.sh" >> $LIVR_FILE
#echo "quote site chmod +x $REMOTE_DEST_LIVRA3/bin/livraison_HOMOLOGATION.sh" >> $LIVR_FILE
#echo "!" >> $LIVR_FILE



echo "put $REMOTE_HOME_BATCH3LOAD_HOMO$REMOTE_DEST_LIVRA3/$BATCH2COMPIL_FILE $REMOTE_HOME_BATCH2LOAD_HOMO/bin/$BATCH2COMPIL_FILE" >> $LIVR_FILE_HOMO
echo "put $REMOTE_HOME_BATCH3LOAD_HOMO$REMOTE_DEST_LIVRA3/$MESSAGE_FILE $REMOTE_HOME_BATCH2LOAD_HOMO/$MESSAGE_FILE" >> $LIVR_FILE_HOMO
echo "put $REMOTE_HOME_BATCH3LOAD_HOMO$REMOTE_DEST_LIVRA3/compil/$COMP_INV_OBJ $REMOTE_HOME_BATCH2LOAD_HOMO/bin/$COMP_INV_OBJ" >> $LIVR_FILE_HOMO
echo "put $REMOTE_HOME_BATCH3LOAD_HOMO$REMOTE_DEST_LIVRA3/$COMPIL_BATCH $REMOTE_HOME_BATCH2LOAD_HOMO/$COMPIL_BATCH" >> $LIVR_FILE_HOMO
echo "quote site chmod +x $REMOTE_HOME_BATCH2LOAD_HOMO/$COMPIL_BATCH" >> $LIVR_FILE_HOMO
echo "!" >> $LIVR_FILE_HOMO
...
echo "	Ils vont etre livre (ftp) sur la HOMO"
