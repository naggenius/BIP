...
--		  en vérifiant d'abord l'existence du code DPG demandé
--		  ( pack_utile.f_verif_dpg(p_codsg)= true or false )
-- Paramètres 	: p_codsg (IN) code DPG demandé
--                              *******
--                              N******
--                              NN*****
--                              NNN****
...
-- Nom 		:
-- Auteur 	: FAD
-- Description 	: Vérifie que le codsg (ouvert ou fermé) appartient ou pas au périmètre de l'utilisateur
-- Paramètres 	: p_codsg (IN) code DPG demandé (161612 )
--		  p_global (IN) contenant le code périmètre de l'utilisateur
-- Retour	:  retourne 'vrai' si habilitation, 'faux' sinon
--
...
-- Nom 		: verif_perim_me
-- Auteur 	: BSA
-- Description 	: Vérifie perim me RTFE de l'utilisateur
--                si tous les codes du perime me  sont inexistant alors KO
-- Paramètres 	: p_global (IN) contenant le code périmètre de l'utilisateur
...
-- Auteur 	: CMA
-- Description 	: Vérifie que l'utilisateur a le droit de consulter les informations de la ressource passée en paramètres :
--        * la ressource a une situation qui est, au moment de la demande, rattachée à un DPG qui est dans le périmètre ME de l'utilisateur; OU
--        * la ressource est parmi les CP ou subordonnés immédiats, habilités dans le profil Saisie directe de l'utilisateur, s'il existe
-- Paramètres 	: p_ident (IN) Identifiant de la ressource que l'utilisateur souhaite consulter
-- 				  p_global (IN) contenant le code périmètre de l'utilisateur
-- Retour	: message d'erreur si non habilité
--
-- **************************************************************************************
...
-- Paramètres 	: p_clicode (IN) Identifiant du client l'utilisateur souhaite consulter
-- 				  p_perim_mo (IN) contenant le code périmètre mo de l'utilisateur
...
-- Paramètres 	: p_cafi (IN) Identifiant de la ressource que l'utilisateur souhaite consulter
-- 				  p_perim_cafi (IN) contenant le code périmètre cafi de l'utilisateur
-- Retour	: "O" pour oui, "N" pour non.
--
-- **************************************************************************************
...
-- Auteur         : OEL
-- Description    : Vérifie si la direction fait partie du perimetre ME de l'utilisateur
--
-- Paramètres     : p_userid (IN)
--                  p_coddir (IN) code direction sur 2 caractères
...
-- Auteur 	: SEL
-- Date     : 17/03/2015
-- Description 	: Vérifie que l'utilisateur a le droit de consulter les informations sur une ligne BIP:
--
-- Paramètres 	: p_pid (IN) Identifiant de la ligne BIP que l'utilisateur souhaite consulter
-- 				        p_global (IN) contenant le code périmètre de l'utilisateur
-- Retour	: message d'erreur si non habilité
--
-- **************************************************************************************
...
-- FAD PPM 63956 : Retourne le périmètre de Directions ou Branches BIP habilitées
PROCEDURE HABILITATION_SUIVBASE(	P_ID_USER   IN RTFE.USER_RTFE%TYPE,
									P_PERIM  OUT VARCHAR2,
									P_MESSAGE OUT VARCHAR2);
-- FAD PPM 63956 : Fin

-- FAD PPM 63956 : Vérifier les régles de gestion liés au IHM
...
END isPerimMo;
 ...
 -- Variables qui contiennent un BDDPG et le code d'habilitation
  ...
  -- *********************************************************************************
    ...
    if substr(t_codsg,4,4)='****' then --un département
        l_saisi :='dpt';
    else
        if substr(t_codsg,6,2)='**' then --un pôle
            l_saisi :='pole';
        else
...
-------------------------------------------------------------------------------------------------------------------------
-- ******************************************************************************************************************* --
-- FAD PPM 63560 : Reproduction de la procédure verif_habili_me pour vérifier les habilitations des DPG ouverts et fermés
...
BEGIN
    ...
    IF ( (p_airt IS NOT NULL) AND (LENGTH(p_airt)>0) ) THEN
        ...
        IF c_hab_appli%NOTFOUND THEN
            ...
            -- Votre périmètre ne vous permet pas de visualiser les informations sur cette application.
...
-------------------------------------------------------------------------------------------------------------------------
-- =====================================================================================================================
-- Vérification que l'utilisateur a le droit de consulter les informations de la ressource passée en paramètres :
--        * la ressource a une situation qui est, au moment de la demande, rattachée à un DPG qui est dans le périmètre ME de l'utilisateur; OU
--        * la ressource est parmi les CP ou subordonnés immédiats, habilités dans le profil Saisie directe de l'utilisateur, s'il existe
-------------------------------------------------------------------------------------------------------------------------
PROCEDURE verif_perim_ressource( p_ident   IN VARCHAR2,
                                   ...
                                   p_message OUT VARCHAR2 ) IS
    l_msg          varchar2(1024);
    l_chefs_projet varchar2(10000);-- PPM 63485 : augmenter la taille à 4000
    l_chefs_projet_ress varchar2(10000);-- PPM 63485 : augmenter la taille à 4000
...
l_chef_de_projet_ligne  VARCHAR2(6000);-- PPM 63485 : augmenter la taille des chefs de projets à 4000
l_chef_de_projet_user   VARCHAR2(6000);-- PPM 63485 : augmenter la taille des chefs de projets à 4000
...
-- ******************************************************************************************************************
    ...
    EXCEPTION
         WHEN NO_DATA_FOUND THEN -- p_out := 'N' --> Vous n'êtes pas habilité à cette direction
         p_out := 'N';
         RETURN p_out;
...
BEGIN
	...
	ELSE
		...
		ELSE
			ID_SPECIFIQUE := NULL;-------Cas non spécifique, poursuivre cas général
			CAS := 'GENERAL';
		END IF;
	END IF;
	...
	IF ID_SPECIFIQUE IS NOT NULL THEN
		...
		EXCEPTION WHEN NO_DATA_FOUND THEN
			CAS := 'GENERAL';
			-------Cas non spécifique, poursuivre cas général
		END;
	END IF;
...
BEGIN
	...
	IF P_PERIM_ME IS NOT NULL AND P_CODSG IS NOT NULL THEN
		...
		IF P_HABILITATION NOT LIKE '00%' AND P_HABILITATION NOT LIKE '%,00%' THEN
			...
			END LOOP;
			-- FAD PPM 63956 : on enlève le dernier OR ajouté et on ferme la parenthèse
