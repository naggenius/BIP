...
-- -----------------------------------------------------------------------------------------------
-- 
-- Ce package contient les procédures qui permettent le traitement de répartition des réestimés
--
-- Crée        le 22/02/2006 par DDI
--
-- Modifié le 16/05/2006 par DDI : Ajout du consommé au reestimé des lignes t9.
-- Modifié le 18/05/2006 par DDI : Ajout de blocs exception sur la boucle des lignes t9..
-- Modifié le 19/05/2006 par DDI : Problèmes d'arrondis sur l_reestime.
...
-- Modifié le 07/03/2007 par EVI : Retours sur l'homologation du lot 6.2 - optimisation
--*************************************************************************************************
--
...
BEGIN
    ...
    -- ------------------------------------------------------------------------------------------------------
    -- Alimentation des reestimes dans la table BUDGET
    -- ------------------------------------------------------------------------------------------------------
...
L_STATEMENT	VARCHAR2(128);
 ...
 l_reestime	INTEGER;
 v_reestime	NUMBER(12,2);
 cpt1		INTEGER;

 consori	CONSOMME.CUSAG%TYPE; -- consomme origine
 reestimori 	BUDGET.reestime%TYPE; -- reestime origine
...
--
	CURSOR cur_ligne_ori(c_table_rep CHAR,
			...
			SELECT lb.pid, nvl(b.reestime,0) reestimori, nvl(c.cusag,0) consori
...
WHILE l_moismens<= l_mois LOOP
 ...
 -- Remet à zéro tous les arbitrés des lignes T9 qui sont reparti pour le mois en cours.
    UPDATE BUDGET
    SET  reestime = 0, apdate = l_date, flaglock = DECODE(flaglock, 1000000, 0, flaglock + 1)
	...
	L_STATEMENT:='Remise à zéro des reestimés des lignes T9';
...
-- Boucle sur les tables de répartition.
	FOR curseur_tab IN cur_table LOOP
		...
		Trclog.Trclog( P_HFILE, L_STATEMENT );
		 ...
		 rafori := 0;
		 rafori := curseur_ligne_ori.reestimori - curseur_ligne_ori.consori;

L_STATEMENT := 'Ligne origine = ' || curseur_ligne_ori.pid || ' rafori: ' || rafori || ' reestimori ' ||curseur_ligne_ori.reestimori;
Trclog.Trclog( P_HFILE, L_STATEMENT );
			...
			IF (rafori>=0) THEN
			   ...
			   FOR curseur_rjh_conso IN cur_rjh_conso(curseur_ligne.pid,curseur_ligne_ori.pid, l_annee)
			   	   LOOP
			   	   v_reestime:= v_reestime + curseur_rjh_conso.consom;
...
-- Cette modif est indispensable pour le cas ou une ligne disparait d'une table de répartition en cours d'année.
	 IF(l_moismens = l_mois ) THEN
			   v_reestime:=v_reestime + (rafori * curseur_ligne.tauxrep);
	 END IF;

			   ELSE IF(rafori<0) THEN
			   			IF(curseur_ligne_ori.reestimori=0) THEN v_reestime:=0; END IF;

			   			IF(curseur_ligne_ori.reestimori>0) THEN
						 /*  Dans ce cas on dois retrouver le mois J qui est le dernier mois ou le consomme n'est pas supérieur a l'arbitré */

					  raf:=curseur_ligne_ori.reestimori;
					  bool:=0;
...
END IF;
							...
							-- Si il n'y a pas de consomme a ventiler.
							 ...
							 v_reestime:=curseur_ligne_ori.reestimori*tauxR;
							...
							WHERE pid=curseur_ligne.pid
							 ...
							 and codrep=curseur_tab.codrep;
							  ...
							  reste:=curseur_ligne_ori.reestimori - r_conso_ori;

							  v_reestime:=r_conso + reste * tauxR;

							END IF;

--L_STATEMENT := dd_flag || ' __RAF= ' || raf || ' r_conso= ' || r_conso || ' reste= ' || reste || ' tauxrep= ' || tauxR ||' dateR= ' || dateR;
--Trclog.Trclog( P_HFILE, L_STATEMENT );
						 END IF;
			   		END IF;
			END IF;

			L_STATEMENT := 'INSERT - pid: ' || curseur_ligne.pid || ' reestime: ' ||v_reestime ;
			Trclog.Trclog( P_HFILE, L_STATEMENT );
			...
			-- Fait la mise à jour dans budget en ajoutant les arbitrés aux arbitrés existants.
                    	...
                    	UPDATE BUDGET
                    	SET  reestime =reestime + v_reestime, apdate = l_date, flaglock = DECODE(flaglock, 1000000, 0, flaglock + 1)
                    	WHERE  pid = curseur_ligne.pid
						and annee=l_annee;
...
END LOOP; -- fin while sur les mois !
	...
	Trclog.Trclog( P_HFILE, 'Nombre de table traitées pour les reestimés : ' || cpt1 );
