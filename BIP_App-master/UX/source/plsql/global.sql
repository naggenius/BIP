...
FUNCTION liste_chefs_projet(p_codeRess IN VARCHAR2, p_listeCp IN OUT ident_table ,p_listeParCP  IN OUT VARCHAR2, p_listeTotale  IN OUT VARCHAR2, p_message  OUT VARCHAR2) RETURN ident_table;
--Fin PPM 61776
--PPM 62551 : début
...
l_listeCp VARCHAR2(6000);--PPM 63485 : augmenter la taille à 4000
...
IF INSTR(p_listeCodeRess,'*') >0 THEN
    ...
    l_exist:=true;
      ...
      IF INSTR(l_cp,'*') = l_len THEN
        ...
        if l_listeCp || '^' = '^' then
         ...
         if not l_exist then
          l_cpident := liste_chefs_projet(l_codeRess, l_cpident, l_listeParCP, l_listeCp, l_msg);--PPM 63485
        end if;
      ELSE
       --DBMS_OUTPUT.PUT_LINE('HERE1');
        if l_listeCp || '^' = '^' then
         ...
         --DBMS_OUTPUT.PUT_LINE('HERE4');
             if REGEXP_COUNT(l_listeCp, ',') > 664 OR length(l_listeCp) > 3994 then--PPM 63485
          --  if(REGEXP_COUNT(p_listeParCP, ',') > 398 OR REGEXP_COUNT(p_listeTotale, ',') > 664 OR length(p_listeTotale) > 3994 )
             goto end_loop;--PPM 63485
             else
                -- QC 1874 : on test si un CP existe déjà dans la liste
    ...
    ELSE
         ...
         -- replace ; par , au cas où la liste CHEF_PROJET est remontée de la table RTFE.
      --DBMS_OUTPUT.PUT_LINE('HERE5'||p_listeCodeRess);
      l_listeCpTotale := SPLIT(replace(p_listeCodeRess,';',','), ',');
      FOR i IN 1..l_listeCpTotale.COUNT LOOP
    l_listeParCP := '';
    l_exist:=true;
...
FUNCTION liste_chefs_projet(p_codeRess IN VARCHAR2, p_listeCp IN OUT ident_table,p_listeParCP  IN OUT VARCHAR2, p_listeTotale  IN OUT VARCHAR2, p_message  OUT VARCHAR2) RETURN ident_table IS

l_ident varchar2(10);
l_cpident_total ident_table:=ident_table();
l_element varchar2(10);
t_cpident ident_table:=ident_table();
l_listeTotale varchar2(6000);--PPM 63485 : augmenter la taille à 4000
i number;
j number;
...
--initialisation
i:=1;
if p_listeCp is null then
  p_listeCp :=ident_table();
  p_listeCp.extend;
  if p_listeCp.COUNT<2  then
  p_listeCp(1) := p_codeRess;
  end if;
end if;
--PPM 63485
if p_listeParCP || '^' = '^' then
  p_listeCp.DELETE;
  p_listeCp :=ident_table();
  p_listeCp.extend;
  if p_listeCp.COUNT<2  then
...
--Fin PPM 63485
 BEGIN
 --if REGEXP_COUNT(p_listeTotale, ',') < 665 then 
 t_cpident.DELETE;
 j:=1;
 l_listeTotale:='';
  FOR n IN 1..p_listeCp.COUNT LOOP
  l_ident := p_listeCp(n);
        FOR curseur IN (select distinct(st.ident),st.cpident from situ_ress st, datdebex d
           where st.cpident =l_ident
           and st.ident <> st.cpident  -- pour ne pas avoir dans la liste un CP en double et éviter une boucle infinie
           and (select count(ident) from situ_ress  where cpident = st.ident
           --on ramène que les ressources N-1 (qui sont des chefs de projet) actifs dans l'année fonctionnelle
                AND (TRUNC(datsitu,'YEAR') <=TRUNC(d.DATDEBEX,'YEAR') OR datsitu IS NULL)
                AND (TRUNC(datdep,'YEAR')>=TRUNC(d.DATDEBEX,'YEAR') OR datdep IS NULL)
                )>0
                --on ramène que les chefs de projet actifs dans l'année fonctionnelle
           AND (TRUNC(st.datsitu,'YEAR') <=TRUNC(d.DATDEBEX,'YEAR') OR st.datsitu IS NULL)
           AND (TRUNC(st.datdep,'YEAR')>=TRUNC(d.DATDEBEX,'YEAR') OR st.datdep IS NULL))
        LOOP
          --l'ident est un CP, on le met dans la liste
          l_element := curseur.ident;
          if t_cpident.EXISTS(l_element) = false 
          
           -- QC 1874 : on test si un CP existe dejaÂ dans la liste
            and INSTR(','||p_listeTotale||',', ','||l_element||',')=0 then
            t_cpident.extend;
            t_cpident(j) := curseur.ident;
            l_cpident_total.extend;
            l_cpident_total(i) := t_cpident(j);
              -- Limiter la taille de la liste des chefs de projet a 4000 caracteres.
               if p_listeTotale|| '^' = '^' then
                  p_listeTotale := t_cpident(j);
                else
                -- PPM 63485 : tronquer la chaine des CPs à 666 soit 666*(5 caractères completer par des 0 à gauche (dans UserBip.java) + 1 virgule) = 3996) 
                  if(REGEXP_COUNT(p_listeParCP, ',') > 398 OR REGEXP_COUNT(p_listeTotale, ',') > 664 OR length(p_listeTotale) > 3994 ) then --PPM 63485
                  p_listeParCP :='';
                     RETURN l_cpident_total;
                  else
                    -- QC 1874 : on test si un CP existe déjà dans la liste
                    if INSTR(','||p_listeTotale||',', ','||t_cpident(j)||',' ) = 0 then
                      l_listeTotale := p_listeTotale || ',' || t_cpident(j);
                      p_listeTotale := l_listeTotale;
                      p_listeParCP := p_listeParCP || ',' || t_cpident(j);--PPM 63485
                    end if;
                  end if;
               end if;
            i := i+1;
 ...
 IF t_cpident.COUNT < 1 THEN
  RETURN l_cpident_total;
 ELSE
  RETURN liste_chefs_projet(p_codeRess,t_cpident, p_listeParCP , p_listeTotale, p_message);
END IF;
--END if;
END;
END liste_chefs_projet;
--Fin PPM 61776
