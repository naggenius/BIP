...
# Nom du fichier :ligne_bip_maj_masse_dbs.sh
# Objet : maj en masse de ligne bip
#
# Parametres d'entree:
#	$1	Fichier log (facultatif)
#_______________________________________________________________________________
# Creation     : Amine BENHAIDA (13/02/2013)
#_______________________________________________________________________________
################################################################################
...
# ############
	...
	if [ -z $LOG_TRAITEMENT ]
	then
		LOG_FILE=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.ligne_bip_maj_masse_dbs.log
...
log_tmp=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.ligne_bip_maj_masse_dbstmp.txt
statuttmp=0
...
logText "--------------------------------------------------------------------------------" >> $LOG_FILE
logText "Debut de la maj en masse de ligne bip : ligne_bip_maj_masse_dbs.sh" >> $LOG_FILE
...
sqlldr $CONNECT_STRING \
	parfile=$DIR_SQLLOADER_PARAM/Ligne_bip_maj_masse_dbs.par \
	control=$DIR_SQLLOADER_PARAM/Ligne_bip_maj_masse_dbs.ctl \
...
logText "SQLPLUS Exec pack_ligne_bip_maj_masse.maj_masse_ligne_bip_dbs" 	
logText "SQLPLUS Exec pack_ligne_bip_maj_masse.maj_masse_ligne_bip_dbs" >> $LOG_FILE		
sqlplus -s $CONNECT_STRING <<! >> $log_tmp 2>&1
	whenever sqlerror exit failure;
	execute pack_ligne_bip_maj_masse.maj_masse_ligne_bip_dbs ('$id_batch');
!
...
then	
	...
	insert into BATCH_LOGS_BIP values ('ligne_bip_maj_masse_dbs.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,null, 'NOK');
EOF
	sqlplus -s $CONNECT_STRING << EOF
	UPDATE TRAIT_BATCH SET TOP_ANO='O' WHERE ID_TRAIT_BATCH = $id_batch;
	commit;
EOF
	logText  "Problème script ligne_bip_maj_masse_dbs.sh : Appel procédure SQL*PLUS : pack_ligne_bip_maj_masse.maj_masse_ligne_bip_dbs "	
	logText "Code erreur : <$PLUS_EXIT>"
	logText  "Problème script ligne_bip_maj_masse_dbs.sh : Appel procédure SQL*PLUS : pack_ligne_bip_maj_masse.maj_masse_ligne_bip_dbs " >> $LOG_FILE								
	logText "Problème SQL*PLUS lors de l'exécution de la procédure  pack_ligne_bip_maj_masse.maj_masse_ligne_bip_dbs : consulter $LOG_FILE"
	logText "Code erreur : <$PLUS_EXIT>" >> $LOG_FILE
	logText "=== ERREUR : Fin ANORMALE de la maj en masse de ligne : ligne_bip_maj_masse_dbs.sh ===" >> $LOG_FILE
	rm -f $log_tmp
	exit -1;
else
	datefin=`date +'%d/%m/%Y'`
	...
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('ligne_bip_maj_masse_dbs.sh','$datedebut', '$heuredebut','$datefin','$heurefin',null,0,null, 'OK');
EOF
	logText "Fin NORMALE de la maj en masse de ligne : ligne_bip_maj_masse_dbs.sh ===" >> $LOG_FILE
	rm -f $log_tmp
fi
