...
#
# Parametres d'entree :
#	$1	le chemin ET LE NOM du fichier de data, contenant la remontée PMW
#
#_______________________________________________________________________________
...
if [ -z $LOG_TRAITEMENT ]
then
	LOG_FILE=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.stloapmw_bips.log
...
DATA_FILE=$1
LOGLOAD_FILE=$DIR_SQLLOADER_LOG/`date +"%Y%m%d"`.stloapmw_bips
...
then
	logText "Problème SQL*LOAD dans batch stloapmw : consulter $LOG_FILE"
