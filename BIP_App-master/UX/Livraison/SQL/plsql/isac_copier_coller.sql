-- CMA 14/01/2011 Ajout et conditionnement de la copie des affectations et du dplacement des consomms
-- CMA 07/02/2011 Fiche 852 Dans le cas du dplacement des consomms, suppressions des consomms de la sous-tache source
-- CMA 08/02/2011 852 Dplacement des consomms  partir du mois de l'anne en cours dans tous les cas
-- CMA 09/02/2011 852 Pas de dplacement des consomms entre la mensuelle dfinitive de dcembre et la clture annuelle
-- CMA 11/02/2011 852 AJout de logs dans le cas de dplacement des consomms
...
--           AND to_number(to_char(c1.cdeb,'MM'))>=TO_NUMBER(p_mois);
            ...
            UPDATE ISAC_CONSOMME C1 SET C1.CUSAG = 0 WHERE C1.SOUS_TACHE = L_SOUS_TACHE
           AND TO_NUMBER(TO_CHAR(C1.CDEB,'MM'))>=TO_NUMBER(P_MOIS);
