...
-- QUI       	QUAND       	QUOI
-- -------------------------------------------
--  NBM	26/09/2003	cration 
--  PJO 05/03/2004	Modification sur 4 carcatres des PID
--  BAA 13/09/2005      Ajout de l'argument affecter pour affecter les ressources ou ne pas les affects
...
--  BAA 30/12/2005      Optimisation des requtes
--  CMA 14/01/2011      852 option de dplacement des consomms
--  CMA 08/02/2011      852 suppression des consomms copis aprs copie
--  CMA 08/02/2011		852 Dplacement des consomms  partir du mois de l'anne en cours dans tous les cas
--  CMA 09/02/2011      852 Pas de dplacement des consomms entre la date de mensuelle dfinitive de dcembre et la clture annuelle
--  CMA 11/02/2011		852 Ajout de logs lors du dplacement des consomms
...
--*********************************************************************
-- Liste des lignes  copier : elles ont au moins une tape
-- *******************************************************************
   PROCEDURE lister_pid1( p_userid  IN VARCHAR2,
                          p_curseur IN OUT RefCurTyp
                             );
--*********************************************************************
-- Liste des lignes  renseigner : elles n'ont aucune tape
-- *******************************************************************
   PROCEDURE lister_pid2( p_userid  IN VARCHAR2,
                          p_curseur IN OUT RefCurTyp
                             );
--********************************************************************************************************************
-- Procdure de copie de la structure d'une ligne BIP vers une autre ligne BIP qui n'a aucune structure
-- paramtres : 	- p_pid1 : ligne BIP  copier
--			- p_pid2 : ligne BIP qui dot tre renseign
-- *******************************************************************************************************************
...
--*********************************************************************
-- Liste des ressources dstinations
-- *******************************************************************
...
--********************************************************************************************************************
-- Procdure de copie les affectations d'une ressource  une autre ressource
-- paramtres : 	- p_pid1 : ressource origine
--			        - p_pid2 : ressource dstination
-- *******************************************************************************************************************
...
BEGIN
    ...
    IF(p_affecter = 'AVEC')THEN
	   ...
	   WHERE  e1.ecet=e2.ecet
          ...
          -- Changes for Issue PPM 64639 -- starts
--        DELETE FROM ISAC_CONSOMME c1
--        WHERE c1.pid=p_pid1
--           AND to_number(to_char(c1.cdeb,'MM'))>=TO_NUMBER(p_mois);
           
            UPDATE ISAC_CONSOMME C1 SET C1.CUSAG = 0 WHERE C1.PID = P_PID1
           AND TO_NUMBER(TO_CHAR(C1.CDEB,'MM'))>=TO_NUMBER(P_MOIS);
           -- Changes for Issue PPM 64639 -- ends
