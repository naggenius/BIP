...
--ABN PPM 63482 - Ajout p_dpcopi
 ...
 --FAD PPM 63826 : Fin
 --SPRINT 6 : BIP 335
PROCEDURE verifier_ProjLigneLink(p_icpi      IN  PROJ_INFO.ICPI%TYPE,
                                 p_table_typo       OUT  ARRAY_TABLE,--Pack_Utile.t_array,
                                 p_message   OUT VARCHAR2         );
  --SPRINT 6 : BIP 335
...
/*BIP 335 new procedure to check the project presence */
PROCEDURE verifier_ProjLigneLink(p_icpi      IN  PROJ_INFO.ICPI%TYPE,
                                 p_table_typo       OUT  ARRAY_TABLE,--Pack_Utile.t_array,
...
BEGIN
    p_message:='PIDACTIVE';
      ...
      BEGIN
          ...
          SELECT pid BULK COLLECT INTO p_table_typo FROM ligne_bip 
          WHERE  icpi=p_icpi AND
          ( ADATESTATUT IS NULL OR (TRUNC(ADATESTATUT,'YEAR')>=trunc((SELECT CMENSUELLE FROM DATDEBEX),'YEAR')) )
          AND typproj = '1' and arctype = 'T1';
               
          IF p_table_typo.count = 0 THEN
          p_message := 'PIDINACTIVE';
          END IF;
     ...
     init_donnees_ca(l_codcamo5, l_clibca5, l_cdfain5, l_datvalli5);
      ...
      BEGIN
        ...
        maj_proj_info_logs(p_icpi, l_user, 'RESPVAL5', '', p_respval5, 'Création du responsable de la validation du lien avec CA');
          -- HMI : PPM 61919 chapitre 6.8
        --PPM 65186 : Adding RTRIM function to remove space from axemetier -- START
        maj_proj_info_logs(p_icpi, l_user, 'PROJAXEMETIER', '', RTRIM(p_projaxemetier), 'Création de l''axe métier PROJET');
        --PPM 65186 : Adding RTRIM function to remove space from axemetier -- END
...
-- BSA 1123
     ...
     p_message := l_msg;
		  ...
		  -- CREATE THE NEW MESSGE FOR BIP 317 userstory 
            WHEN DUP_VAL_ON_INDEX THEN
           Pack_Global.recuperer_message(21323, NULL, NULL, NULL, l_msg);
		...
		END;
         ...
         UPDATE PROJ_INFO SET icpi      = p_icpi,
                  ...
                  deanre        = TO_DATE( p_deanre, 'YYYY')
                              ...
                              --PPM 65186 : Adding RTRIM function to remove space from axemetier -- START
                              , PROJAXEMETIER=RTRIM(p_projaxemetier)
                              --PPM 65186 : Adding RTRIM function to remove space from axemetier -- END
                               WHERE icpi = p_icpi
...
-- BSA 1123
     ...
     -- HMI : PPM 61919 chapitre 6.8
    --PPM 65186 : Adding RTRIM function to remove space from axemetier -- START
    maj_proj_info_logs(p_icpi, l_user, 'PROJAXEMETIER', l_old_projaxemetier, RTRIM(p_projaxemetier), 'Modification de l''axe metier projet');
