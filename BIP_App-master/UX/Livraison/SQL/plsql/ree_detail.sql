...
-- ------------------------------------------------------------------------
   ...
   -- FAD PPM 65042 : Duplication de la fonction tmp_ree_detail_seq pour ajouter la gestion des DPG générique
   FUNCTION tmp_ree_detail_seq2 (p_codsg IN VARCHAR2,
				                P_CODE_SCENARIO  IN REE_SCENARIOS.CODE_SCENARIO%TYPE,
 ...
 FUNCTION tmp_ree_detail_seq (p_codsg IN ligne_bip.codsg%TYPE,
   ...
   FUNCTION TMP_REE_DETAIL_SEQ2 (P_CODSG IN VARCHAR2,
    ...
    st_dpg_gen NUMBER(7);
      ...
      cursor l_c_a(c_codsg CHAR,c_code_scenario CHAR, C_P_PERIME CHAR) IS
	   SELECT
       	RR.CODSG CODSG,
       	SI.LIBDSG LIBDSG,
       	RS.CODE_SCENARIO CODE_SCENARIO,
	   ...
	   FROM
	     ...
	     STRUCT_INFO SI,
	     DATDEBEX D,
       (SELECT CODSG FROM STRUCT_INFO WHERE CODSG >= TO_NUMBER(REPLACE(c_codsg, '*', '0'))
        AND CODSG <= TO_NUMBER(REPLACE(C_CODSG, '*', '9'))
        AND CODSG IN (SELECT CODSG FROM VUE_DPG_PERIME_ALL WHERE INSTR(C_P_PERIME, CODBDDPG) > 0)) DPG_GEN
	  WHERE
         RR.CODSG=DPG_GEN.CODSG
	     and RA.CODSG=RR.CODSG
	     and SI.CODSG=RR.CODSG
	     and RS.CODSG=RR.CODSG
	     and RS.CODE_SCENARIO=c_code_scenario
     ORDER BY NOM,CODE_ACTIVITE;
	  ...
	  CURSOR SS_TRAIT(C_CODSG CHAR,C_CODE_SCENARIO CHAR, C_P_PERIME CHAR) IS
        SELECT DISTINCT lb.codsg, PP.TIRES, PP.RNOM, PP.RPRENOM
        FROM PROPLUS PP, LIGNE_BIP LB, REE_SCENARIOS RS, SITU_RESS SR, DATDEBEX D,
        ...
        and pp.tires = sr.ident
        AND PP.DATDEP = SR.DATDEP
        and sr.codsg <> DPG_GEN.CODSG
        and lb.codsg = DPG_GEN.CODSG
        and rs.code_scenario = c_code_scenario
        and TO_NUMBER(TO_CHAR(sr.datdep,'yyyy')) >= TO_NUMBER(TO_CHAR(d.datdebex,'yyyy'))
        and exists ( select 1
                    from REE_ACTIVITES_LIGNE_BIP r
                    where r.codsg = DPG_GEN.CODSG
                    and r.pid = PP.FACTPID);
   ...
   BEGIN
     ...
     BEGIN
      ...
      --Création automatique ressource de type S pour le DPG s'il n'existe pas
	  --SELECT 'OUI' INTO EXISTE_TYPE FROM REE_RESSOURCES WHERE CODSG=p_codsg AND TYPE='S' and rownum = 1;
    SELECT 'OUI' INTO EXISTE_TYPE FROM REE_RESSOURCES WHERE CODSG >= TO_NUMBER(REPLACE(P_CODSG, '*', '0'))
    AND CODSG <= TO_NUMBER(REPLACE(P_CODSG, '*', '9'))
    AND CODSG IN (SELECT CODSG FROM VUE_DPG_PERIME_ALL WHERE INSTR(p_perime, codbddpg) > 0)
    AND TYPE='S' and rownum = 1;
      EXCEPTION
      ...
      IF(EXISTE_TYPE = 'NON') THEN
              ...
              OPEN ss_trait(p_codsg,p_code_scenario, p_perime);

              LOOP

              fetch ss_trait into st_dpg_gen, st_ident, st_nom, st_prenom;
  
              IF ss_trait%NOTFOUND THEN
                EXIT;
              END IF;
              
              INSERT INTO REE_RESSOURCES(CODSG, TYPE, IDENT, RNOM, RPRENOM) values (st_dpg_gen, 'S', st_ident, st_nom , st_prenom);
      ...
      OPEN l_c_a(p_codsg,p_code_scenario, p_perime);
