...
CREATE OR REPLACE PACKAGE BODY     pack_ligne_bip_pp
AS
...
-- SEL 28/05/2014 PPM 58143 8.6
    ...
    THEN
                 ...
                 INSERT INTO TRAIT_BATCH_RETOUR
                     ...
                     VALUES (p_id_batch,p_ligne.pnom 
                     /* the below commented values are replaced with pnom as per BIP 8 defect changes*/
   ...
   IF(p_ligne.dbs IS NULL) THEN
          ...
          IF(l_dbs_obligation = 'OUI') THEN
             ...
             IF (l_dbs_obligatoire) THEN
                   ...
                   l_erreur := 1;
                     ...
                     INSERT INTO TRAIT_BATCH_RETOUR
                            (ID_TRAIT_BATCH,  DATA, ERREUR )
                     VALUES (p_id_batch,p_ligne.pnom, 'Le DBS est obligatoire et n''a pas été fourni'
                            );
            END IF;
        ...
        -- le DBS est non null
               ...
               EXCEPTION
                 ...
                 INSERT INTO TRAIT_BATCH_RETOUR
                          ...
                          VALUES ( p_id_batch,p_ligne.pnom, 'Le DBS fourni n''est pas autorisée'
                                 );
     ...
     END IF;
            ...
            THEN
               ...
               THEN
                  ...
                  INSERT INTO TRAIT_BATCH_RETOUR
                              (ID_TRAIT_BATCH,  DATA, ERREUR )
                       VALUES ( p_id_batch,p_ligne.pnom, 'Annee vide ou proposé vide'
                              );
               END IF;
               --BIP-155 CHANGES
                IF ( isNumeric ( p_ligne.ANNEE ) = FALSE OR INSTR(p_ligne.ANNEE,',') > 0  OR INSTR(p_ligne.ANNEE,'.') > 0 or length(p_ligne.ANNEE) <> 4 ) THEN
                   ...
                   -- t_presence_retour := TRUE;
                    ...
                    VALUES ( p_id_batch, p_ligne.pnom, 'L''année budget mentionnée est incorrecte'
                        );
                END IF;
               ...
               THEN
                  ...
                  THEN
                     ...
                     INSERT INTO TRAIT_BATCH_RETOUR
                                 (ID_TRAIT_BATCH,  DATA, ERREUR )
                          VALUES (p_id_batch,p_ligne.pnom, 'Annee incorrecte'
                                 );
                  ...
                  THEN
                     ...
                     INSERT INTO TRAIT_BATCH_RETOUR
                                 (ID_TRAIT_BATCH,  DATA, ERREUR )
                          VALUES (p_id_batch,p_ligne.pnom, 'budget incorrect'
                                 );
                  END IF;
            ...
            THEN
               ...
               EXCEPTION
                  ...
                  THEN
                     ...
                     INSERT INTO TRAIT_BATCH_RETOUR
                                 (ID_TRAIT_BATCH,  DATA, ERREUR )
                          VALUES (p_id_batch,p_ligne.pnom, 'projet inexistant'
                                 );
               END;
            END IF;
              ...
              -- IF (p_ligne.arctype != 'T1' )
               ...
               EXCEPTION
                  ...
                  THEN
                     l_erreur := 1;
                           ...
                           INSERT INTO TRAIT_BATCH_RETOUR
                                       (ID_TRAIT_BATCH,  DATA, ERREUR )
                                VALUES (p_id_batch,p_ligne.pnom, 'DP inconnu'
                                       );
                  END;
                  ...
                  l_erreur := 1;
                           ...
                           INSERT INTO TRAIT_BATCH_RETOUR
                                       (ID_TRAIT_BATCH,  DATA, ERREUR )
                                VALUES (p_id_batch,p_ligne.pnom, 'Le DP indiqué n''est pas autorisé pour une ligne non GT1, compte-tenu de la Direction principalement concernée, associée à ce DP'
                                       );
                  END IF;
               /*
               ...
               ELSE
                  ...
                  THEN
                     ...
                     INSERT INTO TRAIT_BATCH_RETOUR
                                 (ID_TRAIT_BATCH,  DATA, ERREUR )
                          VALUES (p_id_batch,p_ligne.pnom, 'Ligne T1 : le DP de la ligne et du projet associé doit être différent de 0 '
                                 );
                  END IF;
                  ...
                  THEN
                     BEGIN
                        ...
                        THEN
                           ...
                           INSERT INTO TRAIT_BATCH_RETOUR
                                       (ID_TRAIT_BATCH,  DATA, ERREUR )
                                VALUES (p_id_batch,p_ligne.pnom , 'Le dossier projet est inactif'
                                       );
                        END IF;
                     EXCEPTION
                        WHEN NO_DATA_FOUND
                        THEN
                           ...
                           INSERT INTO TRAIT_BATCH_RETOUR
                                       (ID_TRAIT_BATCH,  DATA, ERREUR )
                                VALUES (p_id_batch,p_ligne.pnom , 'Le dossier projet n''existe pas'
                                       );
                     END;
                  END IF;
                  ...
                  THEN
                     ...
                     EXCEPTION
                        ...
                        THEN
                           ...
                           INSERT INTO TRAIT_BATCH_RETOUR
                                       (ID_TRAIT_BATCH,  DATA, ERREUR )
                                VALUES (p_id_batch,p_ligne.pnom , 'Le dossier projet et le projet de la ligne ne sont pas lié'
                                       );
                     END;
                  END IF;
          ...
          IF (l_erreur = 0) THEN
             IF ( p_ligne.codcamo = 66666 AND (p_ligne.arctype = 'T1' OR trim(p_ligne.typproj) = '7' OR trim(p_ligne.typproj) = '9')) then
                ...
                INSERT INTO TRAIT_BATCH_RETOUR
                            (ID_TRAIT_BATCH,  DATA, ERREUR )
                     VALUES (p_id_batch,p_ligne.pnom , 'Codcamo doit etre different de ' || p_ligne.codcamo
                            );
            END IF;
            END IF;
              ...
              THEN
                   ...
                   EXCEPTION
                      ...
                      THEN
                       ...
                       INSERT INTO TRAIT_BATCH_RETOUR
                                   (ID_TRAIT_BATCH,  DATA, ERREUR )
                            VALUES (p_id_batch,p_ligne.pnom , 'Codcamo inexistant'
                                   );
                   ...
                   IF (l_erreur = 0) THEN
                        ...
                        IF t_date_fermeture IS NOT NULL THEN
                            IF TO_CHAR(t_date_fermeture,'YYYYMM') <= TO_CHAR(SYSDATE,'YYYYMM') THEN
                                ...
                                INSERT INTO TRAIT_BATCH_RETOUR
                                       (ID_TRAIT_BATCH,  DATA, ERREUR )
                                VALUES (p_id_batch,p_ligne.pnom , 'Le CA payeur est fermé'
                                       );
           ...
           END IF;
            ...
            THEN
               ...
               INSERT INTO TRAIT_BATCH_RETOUR
                           (ID_TRAIT_BATCH,  DATA, ERREUR )
                    VALUES (p_id_batch,p_ligne.pnom , 'Metier'
                           );
            END IF;
     ...
     --DBMS_OUTPUT.PUT_LINE('1 IL FAUT CONTROLER');
                    ...
                    INSERT INTO TRAIT_BATCH_RETOUR
                           (ID_TRAIT_BATCH,  DATA, ERREUR )
                    VALUES (p_id_batch,p_ligne.pnom , 'Rubrique(s) manquante(s) sur le DPG'
                           );
   ...
   /* bip 8 changes
   QC 1993 and 1994 : checking ligne_axemetier conditions after satisfying all other conditions for isinerting the ligne into DB
   */
           if(l_erreur = 0) then 
           
            
              PACK_LIGNE_BIP_MAJ_MASSE.CHECK_LINK_LIGNEAXEMETIER2(p_ligne,l_erreur,p_id_batch);
              --insr_batch_retour(l_erreur,'tested120','outside CHECK_LINK_LIGNEAXEMETIER2');
    ...
    /* bip 8 changes*/
            ...
            THEN
               BEGIN
                  ...
                  END IF;
				  --FAD PPM 63410: Fin
          -- Bip us8 changes, adding lineaxisbusiness1,2
                  INSERT INTO ligne_bip
                              (pid, pcle, flaglock, topfer, pcactop, pconsn1, pdecn1, petat, dpcode, pnom, typproj, pdatdebpre,
                               ...
                               pzone, metier, pobjet,CODREP,sous_type,astatut,lineaxisbusiness1,lineaxisbusiness2
                              )
                       VALUES (RTRIM (l_pid), l_pcle, 0, 'N', 'O', 0, 0, 'M', p_ligne.dpcode, p_ligne.pnom, p_ligne.typproj, p_ligne.pdatdebpre,
                              ...
                              p_ligne.lineaxisbusiness1,p_ligne.lineaxisbusiness2);
                  --ABN PPM 59138 - FIN
           -- Bip us8 changes, adding LOGS FOR lineaxisbusiness1,2
                  pack_ligne_bip.maj_ligne_bip_logs (RTRIM (l_pid), 'CREATION MASSE', 'Axe métier Ligne 1', NULL, p_ligne.lineaxisbusiness1, 'Création de la ligne BIP');
                  pack_ligne_bip.maj_ligne_bip_logs (RTRIM (l_pid), 'CREATION MASSE', 'Axe métier Ligne 2', NULL, p_ligne.lineaxisbusiness2, 'Création de la ligne BIP');
            -- BIP 549
            	  pack_ligne_bip.maj_ligne_bip_logs (RTRIM (l_pid), 'CREATION MASSE', 'Code client principal', NULL, p_ligne.clicode, 'Création de la ligne BIP'); 
               ...
               EXCEPTION
                  ...
                  THEN
                    l_liberreur := SQLERRM;
                     ...
                     INSERT INTO TRAIT_BATCH_RETOUR
                                 (ID_TRAIT_BATCH,  DATA, ERREUR )
                          VALUES (p_id_batch,p_ligne.pnom , l_liberreur
                                 );
               END;
         ...
         LOOP
            ...
            THEN
               BEGIN
                 ...
                 pack_ligne_bip.maj_ligne_bip_logs (RTRIM (l_pid), 'CREATION MASSE', 'Code client principal', NULL, un_bip.clicode, 'Création de la ligne BIP');
