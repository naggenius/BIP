...
---------------------------------------------------------------------------------------------------------
 ...
 (select c.pid as PID, l.codsg, e.ecet CCET, t.acta CCTA, st.acst CCST, TO_CHAR(c.ident,'FM00000') CIRES,
        c.cdeb CDEB, c.cusag CUSAG,'WARNING pour la ligne '||c.pid||', ressource '||TO_CHAR(c.ident,'FM00000')||', activité '||e.ecet||t.acta||st.acst||' et début de période consommée '||c.cdeb||' : le type d''étape est incompatible avec votre entité et/ou la ligne Bip' MOTIF_REJET
    from isac_consomme c, isac_sous_tache st, isac_tache t, isac_etape e, datdebex d,ligne_bip l
    where c.sous_tache=st.sous_tache
    and st.tache=t.tache
    and t.etape=e.etape
...
WHERE rowid NOT IN
  ...
  nnligne := SQL%ROWCOUNT;
	TRCLOG.TRCLOG( L_HFILE, nnligne || ' duplicate ligne(s) supprimé(s) from TMP_REJETMENS_TAIL');
	TRCLOG.TRCLOG( L_HFILE, '---------------------------------------');
...
BEGIN
	...
	LOOP
		...
		BEGIN
            ...
            'Warning pour la ligne ' || CUR.PID || ' : la référence de demande est absente du lot DPCOPI, ou inexistante ou incompatible avec le DPCOPI');
				...
				BEGIN
					...
					IF L_OBLIGATOIRE = 'O' THEN
						IF L_PROJAXEMETIER IS NULL THEN
							INSERT INTO TMP_REJETMENS_TAIL (PID,CODSG,ECET,ACTA,ACST,IDENT,CDEB,CUSAG,MOTIF_REJET)
              VALUES (CUR.PID,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
              'Warning pour la ligne ' || CUR.PID || ' : la référence de demande est absente du Projet, ou inexistante ou incompatible avec le Projet');
...
/*BIP-34 Monthly control changes -- starts*/
    IF CONTROLE_NIVEAU = 'TACHE' THEN
          ...
          SELECT COUNT(*) INTO nbrC FROM TMP_REJETMENS_TAIL WHERE PID = CUR.PID AND ECET = CUR.ECET AND ACTA = CUR.ACTA AND
            ...
            VALUES (CUR.PID,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
						'Warning : la référence de demande est absente, inexistante ou incompatible avec la ligne',1
						);
            CONTROLE_NIVEAU := '';
					END IF;
				END IF;
		...
		IF CONTROLE_NIVEAU = 'TACHE' THEN
          ...
          SELECT COUNT(*) INTO nbrC FROM TMP_REJETMENS_TAIL WHERE PID = CUR.PID AND ECET = CUR.ECET AND ACTA = CUR.ACTA AND
            ...
            VALUES(CUR.PID, CUR.CODSG, CUR.ECET, CUR.ACTA, CUR.ACST, CUR.IDENT, CUR.CDEB, CUR.CUSAG,
						DECODE(L_RET,
						'A','Warning pour la ligne ' || CUR.PID || ', activité ' || CUR.ECET || CUR.ACTA || ' : la référence de demande est absente, inexistante ou incompatible avec la ligne',
						'P','Warning pour la ligne ' || CUR.PID || ', activité ' || CUR.ECET || CUR.ACTA || ' : la référence de demande est déjà mentionnée sur un Projet Bip, ce qui est incohérent',
						'D','Warning pour la ligne ' || CUR.PID || ', activité ' || CUR.ECET || CUR.ACTA || ' : la référence de demande est déjà mentionnée sur un lot DPCOPI, ce qui est incohérent'
