...
PACKAGE     Pack_Ligne_Bip AS
   ...
   TYPE ligne_bip_ViewType IS RECORD ( PID         	LIGNE_BIP.PID%TYPE,
                     ...
                     LIGNE_AXEMETIER1  LIGNE_BIP.LINEAXISBUSINESS1%TYPE,
                     LIGNE_AXEMETIER2  LIGNE_BIP.LINEAXISBUSINESS2%TYPE
									);
  ...
  -- définition du curseur sur la vue VUE_CLIENT_PERIMO SEL 22/05/2014 PPM 58143 8.6
   ...
   PROCEDURE insert_ligne_bip (
                               ...
                               p_userid     	IN VARCHAR2,
                                p_ligneaxemetier1 IN VARCHAR2,
                               p_ligneaxemetier2 IN VARCHAR2,
                               p_pid        	OUT CHAR,
                               p_pcle       	OUT CHAR,
                               p_nbcurseur  	OUT INTEGER,
                               p_message    	OUT VARCHAR2
...
--USER STORY BIP 29 : INTIALISE THE AXEMETIER LIGNE 2
PROCEDURE INITIALISE_AXEMETIER2(P_PID IN VARCHAR2,P_PNOM IN VARCHAR2,P_MSG OUT VARCHAR2);
--END USER STORY BIP 29

--USER STORY BIP 29 : CHECKING THE AXEMETIER LIGNE 2
--QC 2008 TO CHECK VALUES FROM FRONT END
PROCEDURE CHECK_AXEMETIERLIGNE2(P_PID IN VARCHAR2,P_LIGNE2 IN VARCHAR2,P_TYPPROJ IN VARCHAR2,P_ARCTYPE IN VARCHAR2,P_CLICODE IN client_mo.clicode%type,P_CODSG IN struct_info.codsg%type,P_PZONE IN VARCHAR2,
                                P_MSG OUT VARCHAR2,p_type OUT CHAR, p_param_id OUT VARCHAR2);
--END USER STORY BIP 29

--USER STORY BIP 29 : NULLYFYING THE AXEMETIER IN PROJ/DPCOPI
PROCEDURE UPDATE_PRDP_LIGNE2 ( type_msg   IN CHAR, param_id IN VARCHAR2, p_userid IN VARCHAR2, p_message   OUT VARCHAR2 ) ;
--END USER STORY BIP 29

--USER STORY BIP 29 ,QC 2008: INTIALISE THE AXEMETIER LIGNE 2 using the GUI values
PROCEDURE GUI_INITIALISE_AXEMETIER2(P_TYPPROJ IN VARCHAR2,P_ARCTYPE IN VARCHAR2,P_CLICODE IN VARCHAR2,
P_CODSG IN VARCHAR2,P_PZONE IN VARCHAR2,P_MSG OUT VARCHAR2);
--end

PROCEDURE CHECK_DMPLINK(P_PID IN VARCHAR2,P_MSG OUT VARCHAR2,P_OBL OUT CHAR );
...
/* used to check the DMP link using PID(P_PNOM) */
PROCEDURE CHECK_DMPLINK(
    P_PID IN VARCHAR2,
    P_MSG OUT VARCHAR2,
    P_OBL OUT CHAR )
IS
...
BEGIN
    ...
    WHERE code_action = 'AXEMETIER_'
      ||l_direction
    AND code_version = 'LI2'
      ||
           -- QC 1993 and QC 1994 change is done to add a 0 to typproj having only one digit 
      (
      SELECT DECODE(LENGTH(trim(typproj)),1,'0'
        ||trim(typproj),trim(typproj))
        ||upper(trim(arctype))
      FROM ligne_bip
      WHERE pid=P_PID
      )
  ...
  WHEN NO_DATA_FOUND THEN
    ...
    WHEN NO_DATA_FOUND THEN
            ...
            /* condition BIP LIGNE has no link    */
...
/* QC 2008 changes - the conditions for link is being checked with values from GUI as it is update functionality */
PROCEDURE CHECK_DMPLINK_INTERNAL(
    P_TYPPROJ IN VARCHAR2,
    P_ARCTYPE IN VARCHAR2,
    P_CLICODE IN client_mo.clicode%type,
    P_CODSG   IN struct_info.codsg%type,
    P_MSG OUT VARCHAR2,
    P_OBL OUT CHAR ,
    P_DIRECTION OUT client_mo.clidir%type)
IS
l_code_action     ligne_param_bip.code_action%type;
l_valeur          ligne_param_bip.valeur%type;
...
BEGIN
     ...
     BEGIN --BEGIN2
     P_MSG:='LINK';
     --should check the client code from gui
    SELECT CMO.clidir
    INTO P_DIRECTION
    FROM client_mo CMO
    ...
    WHERE code_action = 'AXEMETIER_'
      ||P_DIRECTION
             -- should check the typ proj and arc type from front end not using P_NOM from DB
    AND code_version = 'LI2'
      ||
          -- QC 1993 and QC 1994 change is done to add a 0 to typproj having only one digit 
      DECODE(LENGTH(trim(P_TYPPROJ)),1,'0'
      ||trim(P_TYPPROJ),trim(P_TYPPROJ))
      ||upper(trim(P_ARCTYPE))
    AND actif = 'O';
          EXCEPTION
  WHEN NO_DATA_FOUND THEN
          BEGIN
    
            --should check the codsg from gui
      SELECT SI.coddir
      INTO P_DIRECTION
      FROM struct_info SI
      WHERE SI.codsg = P_CODSG;
      SELECT code_action,
        obligatoire,
        valeur
      INTO l_code_action,
...
IF (p_pid IS NOT NULL) THEN
  ...
  IF(p_duplic = 'OUI') THEN
                 ...
                 OPEN p_curLigne_bip FOR
						SELECT 	lb.PID,
								...
								dp.ACTIF,
								pi.STATUT,
                lb.LINEAXISBUSINESS1,
                lb.LINEAXISBUSINESS2
						FROM LIGNE_BIP         lb,
						TYPE_PROJET       tp,
						TYPE_ACTIVITE     ta,
						CODE_STATUT       cs,
	...
	objet_split := controle_obj (p_pobjet);
        ...
        BEGIN
           ...
           p_ligneaxemetier2);
			...
			-- QC 2005 to add logs while creating the ligneaxemetier1,ligneaxemetier2
            maj_ligne_bip_logs(l_pid, l_user, 'ligneaxemetier1', NULL, p_ligneaxemetier1, 'Création de la ligne BIP');
            maj_ligne_bip_logs(l_pid, l_user, 'ligneaxemetier2', NULL, p_ligneaxemetier2, 'Création de la ligne BIP');
            ...
            -- bip 549
            maj_ligne_bip_logs(l_pid,l_user,'Code client principal',null,p_clicode,'Création de la ligne BIP');
 ...
 l_topfer_clientmo CLIENT_MO.CLITOPF%TYPE;
 --QC 2005 to add logs while updating 
 l_lineaxisbusiness1 LIGNE_BIP.lineaxisbusiness1%type;
 l_lineaxisbusiness2 LIGNE_BIP.lineaxisbusiness2%type;
   ...
   BEGIN
	...
	objet_split := controle_obj(p_pobjet);
        ...
        BEGIN
            ...
            -- QC 2005 to add logs while modification of LINEAXISBUSINESS1,LINEAXISBUSINESS2
            --BIP 549 add logs while modification of CLICODE
            SELECT typproj, arctype, codcamo, topfer, astatut, adatestatut, METIER,airt, dpcode,icpi, codsg, sous_type,pzone,lineaxisbusiness1,lineaxisbusiness2,clicode
            INTO l_typproj, l_arctype, l_codcamo, l_topfer, l_astatut, l_adatestatut, l_metier,l_airt,l_dpcode,l_icpi,l_codsg, l_sous_typo,l_pzone,l_lineaxisbusiness1,l_lineaxisbusiness2,l_clicode
            FROM LIGNE_BIP
            WHERE pid=p_pid
            ...
            SET pnom 		= p_pnom,
				...
				codrep      = p_codrep,
        LINEAXISBUSINESS1 = p_ligneaxemetier1,
        LINEAXISBUSINESS2 = p_ligneaxemetier2
            WHERE pid = p_pid
            AND flaglock = p_flaglock;

             -- On loggue le type, la typologie, le CA payeur, le topfer, le statut, la date de statut
             -- QC 2005 to add logs while modification of LINEAXISBUSINESS1,LINEAXISBUSINESS2
            maj_ligne_bip_logs(p_pid, l_user, 'ligneaxemetier1', l_lineaxisbusiness1, p_ligneaxemetier1, 'Modification de la ligne BIP');
            maj_ligne_bip_logs(p_pid, l_user, 'ligneaxemetier2', l_lineaxisbusiness2, p_ligneaxemetier2, 'Modification de la ligne BIP');
            ...
            -- BIP 549
            maj_ligne_bip_logs(p_pid,l_user,'Code client principal ',l_clicode,p_clicode,'Modification de la ligne BIP');
...
--------------------------------- Fiche 656 --------------------------------------------
-------------------------BIP 592 to change sysdate to Datdebex--------------------------
PROCEDURE recherche_realise_anterieur(p_pid IN VARCHAR2 ,
                                      p_realise OUT VARCHAR2  ) IS

    l_count NUMBER ;
    l_realise NUMBER (12,2) ;
    l_anne BUDGET.ANNEE%TYPE ;
        BEGIN
                  SELECT EXTRACT(YEAR FROM DATDEBEX) INTO l_anne FROM DATDEBEX;
                  p_realise := NULL ;

                  SELECT COUNT(*) INTO l_count FROM CONSOMME cons WHERE cons.ANNEE < l_anne
                  AND TRIM(cons.PID)=TRIM(p_pid) ;

                   IF(l_count>0) THEN
                        SELECT SUM(CUSAG) INTO l_realise  FROM CONSOMME cons WHERE cons.ANNEE < l_anne
                        AND TRIM(cons.PID)=TRIM(p_pid) ;
...
PROCEDURE recherche_arbitre_actuel(p_pid IN VARCHAR2 ,
        ...
        BEGIN
                  ...
                  SELECT EXTRACT(YEAR FROM DATDEBEX) INTO l_anne FROM DATDEBEX;
                    ...
                    select COUNT(*) into L_COUNT from BUDGET BUD where BUD.ANNEE = l_anne
                     AND TRIM(bud.PID)=TRIM(p_pid) ;

                    IF(l_count>0) THEN
                        SELECT SUM(ANMONT) INTO l_arbitre FROM BUDGET bud WHERE bud.ANNEE =l_anne
                         AND TRIM(bud.PID)=TRIM(p_pid) ;
...
BEGIN
	...
	-- FAD PPM 65100 : Suppression du curseur et insertion des données directement depuis le SELECT
	...
	INSERT INTO tmp_conso_dmp (
		...
		(select DISTINCT LB.PID, LB.ICPI from ligne_bip LB, datdebex D
		where (LB.adatestatut IS NULL OR LB.adatestatut >= D.datdebex)
		AND (LB.codsg IN (SELECT codsg FROM vue_dpg_perime where INSTR(NVL(P_PERIME, '00000000000'), codbddpg) > 0 ))) ligne
	...
	INSERT INTO tmp_conso_dmp (
		...
		AND LB.PID = ligne.PID
		AND LB.PID NOT IN (SELECT PID FROM tmp_conso_dmp WHERE l_numseq = numseq)
	);
	--FAD PPM 64056 : Fin

  
  --Changes for BIP_39 -- stars
  
	INSERT INTO tmp_conso_dmp (
		SELECT LB.PID, LB.LINEAXISBUSINESS2, 0, 0, 0, l_numseq
		FROM LIGNE_BIP LB,
		(select DISTINCT LB.PID from ligne_bip LB, datdebex D
		where (LB.adatestatut IS NULL OR LB.adatestatut >= D.datdebex)
		AND (LB.codsg IN (SELECT codsg FROM vue_dpg_perime where INSTR(NVL(P_PERIME, '00000000000'), codbddpg) > 0 ))) ligne
		WHERE LB.LINEAXISBUSINESS2 IS NOT NULL
		AND LB.PID = ligne.PID
		AND LB.PID NOT IN (SELECT PID FROM tmp_conso_dmp WHERE l_numseq = numseq)
	);
  
  --Changes for BIP_39 -- ends
	...
	INSERT INTO tmp_conso_dmp (
      ...
      --FIX FOR Process PPM 65187 -- STARTS
      ISAC_TACHE.SOUS_TACHE as SOUS_TACHE
       --FIX FOR Process PPM 65187 -- ENDS
			FROM ISAC_ETAPE
			JOIN
			(
				SELECT  Decode(substr(stt.AIST,1,2),'FF',substr(stt.AIST,3),stt.PID) FACTPID,tache.PID,tache.TACHEAXEMETIER,tache.ETAPE,
        --FIX FOR Process PPM 65187 -- STARTS
        tache.ACTA,stt.SOUS_TACHE,stt.ACST
        --FIX FOR Process PPM 65187 -- ENDS
				from isac_tache tache,isac_sous_tache stt
				where tache.pid=stt.pid
				AND tache.ETAPE=stt.ETAPE
				AND tache.TACHE=stt.TACHE
			) ISAC_TACHE ON ISAC_ETAPE.PID = ISAC_TACHE.PID AND ISAC_ETAPE.ETAPE = ISAC_TACHE.ETAPE
			LEFT JOIN
        ...
        --FIX FOR Process PPM 65187 -- ENDS
				...
				WHERE CDEB BETWEEN (SELECT DATDEBEX FROM DATDEBEX) AND (SELECT MOISMENS FROM DATDEBEX) GROUP BY DECODE(TACHE.AISTTY, 'FF', TACHE.AISTPID, TACHE.PID), TACHE.ECET, TACHE.ACTA, TACHEAXEMETIER, TACHE.ACST
      ...
      --FIX FOR Process PPM 65187 -- STARTS
      AND CONSOAA.ACST = ISAC_TACHE.ACST
      --FIX FOR Process PPM 65187 -- ENDS
      ...
      --FIX FOR Process PPM 65187 -- STARTS
      AND CONSOEURAA.ACST = ISAC_TACHE.ACST
      --FIX FOR Process PPM 65187 -- ENDS
			-- FAD PPM Corr 64486 : Fin
			LEFT JOIN
        ...
        --FIX FOR Process PPM 65187 -- ENDS
				FROM TACHE
				LEFT JOIN CONS_SSTACHE_RES_MOIS ON CONS_SSTACHE_RES_MOIS.PID = TACHE.PID AND  CONS_SSTACHE_RES_MOIS.ECET = TACHE.ECET AND CONS_SSTACHE_RES_MOIS.ACTA = TACHE.ACTA AND CONS_SSTACHE_RES_MOIS.ACST = TACHE.ACST
				WHERE TO_CHAR(CDEB, 'MM/YYYY') = TO_CHAR((SELECT MOISMENS FROM DATDEBEX), 'MM/YYYY') GROUP BY DECODE(TACHE.AISTTY, 'FF', TACHE.AISTPID, TACHE.PID), TACHE.ECET, TACHE.ACTA, TACHEAXEMETIER, TACHE.ACST
      ...
      --FIX FOR Process PPM 65187 -- STARTS
      AND CONSOMM.ACST = ISAC_TACHE.ACST
      --FIX FOR Process PPM 65187 -- ENDS
			-- FAD PPM Corr 64486 : Fin
			JOIN
			(
				...
				where (LB.adatestatut IS NULL OR LB.adatestatut >= D.datdebex)
				AND (LB.codsg IN (SELECT codsg FROM vue_dpg_perime where INSTR(NVL(P_PERIME, '00000000000'), codbddpg) > 0 ))
			) ligne ON ISAC_TACHE.FACTPID = ligne.PID
			WHERE ISAC_TACHE.TACHEAXEMETIER IS NOT NULL
			AND ISAC_TACHE.FACTPID NOT IN (SELECT PID FROM tmp_conso_dmp WHERE l_numseq = numseq)
		)
		GROUP BY PID, DMPNUM
		--FAD PPM 64056 : Fin
	);
...
--USER STORY BIP 29 to initialise the ligne2 field dynamically using the GUI values
PROCEDURE GUI_INITIALISE_AXEMETIER2(
    P_TYPPROJ IN VARCHAR2,
    P_ARCTYPE IN VARCHAR2,
...
l_param_local LIGNE_BIP.PZONE%TYPE;
BEGIN 
  CHECK_DMPLINK_INTERNAL(P_TYPPROJ,P_ARCTYPE,P_CLICODE,P_CODSG,P_MSG,l_obligatoire,l_direction);
      IF (P_MSG='LINK') THEN --IF1
    ...
    IF ( (l_bbrf = '03' OR l_bbrf_cli = '03' OR l_bbrf_dpg ='03') AND (upper(P_PZONE) = 'TRANSITOIRE' OR upper(P_PZONE) = 'DEVIS')) THEN
                  P_MSG:='SUCCESS';
                ELSE
                  P_MSG:='FAIL';
                END IF;
...
WHEN OTHERS THEN
  P_MSG:='FAIL';
END GUI_INITIALISE_AXEMETIER2;

--USER STORY BIP 29
PROCEDURE INITIALISE_AXEMETIER2(
    P_PID  IN VARCHAR2,
    P_PNOM IN VARCHAR2,
    P_MSG OUT VARCHAR2)
IS
...
l_code_version LIGNE_PARAM_BIP.CODE_VERSION%TYPE;
l_param_local LIGNE_BIP.PZONE%TYPE;
  --GUI pid variable is 30 char
l_pid VARCHAR2(30);

BEGIN 
      BEGIN --BEGIN2
      SELECT NVL2(P_PID,P_PID,P_PNOM) INTO l_pid FROM DUAL;
    SELECT CMO.clidir,
      LB.PZONE
    INTO l_direction,
      l_param_local
  ...
  WHEN NO_DATA_FOUND THEN
    ...
    WHEN NO_DATA_FOUND THEN
      ...
      AND actif         = 'O';
        ...
        FROM vue_clicode_perimo
        WHERE clicode =
          (SELECT clicode FROM ligne_bip WHERE pid = l_pid
          )
        AND codhabili = 'br';
                        EXCEPTION
      WHEN NO_DATA_FOUND THEN
        ...
        FROM vue_dpg_perime
        WHERE codsg =
          (SELECT codsg FROM ligne_bip WHERE pid = l_pid
          )
        AND codhabili = 'br';
                    EXCEPTION
      WHEN NO_DATA_FOUND THEN
    ...
    l_bbrf_cli  := SUBSTR(l_valeur_cli,1,2);
    l_bbrf_dpg  := SUBSTR(l_valeur_dpg,1,2);
    IF ( (l_bbrf = '03' OR l_bbrf_cli = '03' OR l_bbrf_dpg ='03') AND (upper(l_param_local) = 'TRANSITOIRE' OR upper(l_param_local) = 'DEVIS')) THEN
                  P_MSG:='SUCCESS';
                ELSE
                  P_MSG:='FAIL';
                END IF;
...
WHEN OTHERS THEN
  P_MSG:='FAIL';
END INITIALISE_AXEMETIER2;
      
       

PROCEDURE CHECK_AXEMETIERLIGNE2(
    P_PID     IN VARCHAR2,
    P_LIGNE2  IN VARCHAR2,
    P_TYPPROJ IN VARCHAR2,
    P_ARCTYPE IN VARCHAR2,
    P_CLICODE IN client_mo.clicode%type,
    P_CODSG   IN struct_info.codsg%type,
...
BEGIN
     BEGIN --BEGIN2
     --l_direction should come from here
    CHECK_DMPLINK_INTERNAL(P_TYPPROJ,P_ARCTYPE,P_CLICODE,P_CODSG,P_MSG,l_obligatoire,l_direction);
          IF(P_MSG='LINK') THEN --mainif
           
            BEGIN --B
            /* QC 2003 start proceed to check the conditions if dmpnum is equal to ligne2*/
               BEGIN
          SELECT RTRIM(li.LINEAXISBUSINESS2)
          INTO LigneAxeMet2Long
          FROM ligne_bip li
          WHERE pid=P_PID;
          SELECT DISTINCT dmpnum
          INTO l_dmpnum
          FROM DMP_RESEAUXFRANCE
          WHERE dmpnum = P_LIGNE2;
                EXCEPTION
                          WHEN NO_DATA_FOUND THEN 
          IF( NVL(LigneAxeMet2Long,'X') = NVL(P_LIGNE2,'Y') ) THEN --IF1
                       P_MSG:='PROCEED';
                       p_type := 'N';
                       p_param_id:= '0000';
                 ...
                 END;
             /* QC 2003 end*/
                IF(P_MSG = 'LINK')THEN --IF3
                    --IF2
                    IF(upper(l_obligatoire) = 'O' AND (P_LIGNE2 IS NULL OR P_LIGNE2 ='')) THEN
            --pack_global.recuperer_message (21300, NULL, NULL, NULL, P_MSG);
                    P_MSG:='GOBACK';
                    p_type := 'N';
                    p_param_id:= '0000';
            ...
            l_bbrf_dpg := SUBSTR(l_valeur_dpg,1,2);
--                      BEGIN
--                          select pzone  into l_param_local from ligne_bip where pid = P_PNOM;
--                        EXCEPTION
--                          WHEN NO_DATA_FOUND THEN null; 
--                      END;
--                         P_MSG:='La rfrence de demande est absente, inexistante ou incompatible avec la ligne';
                    --1
                    
                    --bif
            IF ( (l_bbrf = '03' OR l_bbrf_cli = '03' OR l_bbrf_dpg ='03') AND (upper(P_PZONE) = 'TRANSITOIRE' OR upper(P_PZONE) = 'DEVIS') AND P_LIGNE2 != '000000') THEN
                       P_MSG:='GOBACK';
                       p_type := 'N';
                       p_param_id:= '0000';
            ELSE
              IF ( l_obligatoire = 'O' AND P_LIGNE2 != '000000') THEN
                            BEGIN
                  /* check condition for the presence of LINEAXISBUSINESS2 in dmpnum column of DMP_RESEAUXFRANCE and DDETYPE = 'C' */
                  SELECT DISTINCT dmpnum
                  INTO l_dmpnum
                  FROM DMP_RESEAUXFRANCE
                  WHERE dmpnum       = P_LIGNE2
      ...
      IF ( P_MSG = 'UNICITY') THEN--1
            
            IF(P_LIGNE2 IS NOT NULL OR P_LIGNE2 <>'' ) THEN--2
        ...
        INTO p_param_id,
          p_type
        FROM PROJ_INFO
        WHERE ProjAxeMetier = P_LIGNE2
        AND rownum          = 1;
              pack_global.recuperer_message (21301, '%s1', p_param_id, NULL, P_MSG);
              EXCEPTION
              WHEN NO_DATA_FOUND THEN
          ...
          INTO p_param_id,
            p_type
          FROM dossier_projet_copi
          WHERE dpcopiaxemetier = P_LIGNE2
          AND rownum            = 1;
                  pack_global.recuperer_message (21302, '%s1', p_param_id, NULL, P_MSG);
...
WHEN OTHERS THEN
  P_MSG:='GOBACK';
END CHECK_AXEMETIERLIGNE2;


--BIP 29 USER STORY
 PROCEDURE UPDATE_PRDP_LIGNE2(
    type_msg IN CHAR,
...
IS
  ...
  IF ( 'D' = TRIM(type_msg) ) THEN
    SELECT dpcopiaxemetier
    INTO l_old_dpcopiaxemetier
    FROM DOSSIER_PROJET_COPI
    WHERE dp_copi = param_id;
    UPDATE DOSSIER_PROJET_COPI
    SET dpcopiaxemetier = NULL
    WHERE dp_copi       = param_id;
    PACK_COPI.maj_dossier_projet_copi_logs (UPPER(param_id), l_user, 'DPCOPIAXEMETIER',l_old_dpcopiaxemetier,NULL,'Mis  vide par contrle d''unicit');
  elsif ( 'P' = TRIM(type_msg) ) THEN
    SELECT ProjAxeMetier
    INTO l_old_projaxemetier
    FROM PROJ_INFO
    WHERE icpi = param_id;
    UPDATE PROJ_INFO SET ProjAxeMetier = NULL WHERE icpi = param_id;
    PACK_PROJ_INFO.maj_proj_info_logs(UPPER(param_id), l_user, 'ProjAxeMetier', l_old_projaxemetier, NULL, 'Mis  vide par contrle d''unicit');
  END IF;
  p_message:='DONE';
EXCEPTION
WHEN OTHERS THEN
  p_message:='NOTDONE';
    END UPDATE_PRDP_LIGNE2;
