...
CREATE OR REPLACE PACKAGE BODY     pack_st_amort AS
   ...
   PROCEDURE update_st_amort (p_pid         IN  ligne_bip.pid%TYPE,
    ...
    L_MOISMENS varchar2(10);
    A_MOISMENS number(10);
	V_CUSG number(10);
...
-- comment the 20923 message FOR 336 USERSTORY 
	...
	END IF;
	  ...
	  -- allow the functional year for 336 userstory  - start
    

	SELECT TO_NUMBER(TO_CHAR(add_months(cmensuelle,-12),'YYYY')),TO_NUMBER(TO_CHAR(CMENSUELLE,'yyyy')) 
	INTO L_MOISMENS,A_MOISMENS 
	FROM datdebex;
	  
   

  /*SELECT to_char(adatestatut,'YYYY') into v_adatestatut
  FROM ligne_bip    
   WHERE  pid = p_pid
   AND flaglock = p_flaglock;*/       
IF p_adatestatut IS NOT NULL THEN
...
--Defect 621 changes 
SELECT NVL(SUM(CUSAG),0) INTO V_CUSG FROM CONS_SSTACHE_RES_MOIS WHERE PID = P_PID AND CDEB > TO_DATE('01/'||p_adatestatut,'DD/MM/YYYY');
exception
...
IF V_CUSG > 0
THEN
pack_global.recuperer_message(21324,NULL,NULL,NULL,l_msg);
--Defect 621 changes remove the status at the end of the message-- l_msg:= l_msg||';'||l_astatut;
raise_application_error( -20000, l_msg);
p_message := l_msg;  
END IF;
 
       
IF NVL(to_number(substr(p_adatestatut,4,length(P_ADATESTATUT))),A_MOISMENS) = A_MOISMENS 
OR (TO_CHAR(to_date(12,'MM'),'MM') = SUBSTR(p_adatestatut,1,2) 
and NVL(TO_NUMBER(SUBSTR(P_ADATESTATUT,4,length(P_ADATESTATUT))),L_MOISMENS) = L_MOISMENS)
AND V_CUSG = 0 THEN
