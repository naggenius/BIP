    ...
    L_LINE varchar2(3) := '---';
     ...
     /*OPEN    p_curseur FOR
      SELECT
        ...
        NVL(TO_CHAR(DATSITU,'dd/mm/yyyy'), L_LINE)
        || L_DELIMITER
        || NVL(TO_CHAR(DATDEP,'dd/mm/yyyy'), L_LINE)
        || L_DELIMITER
        || NVL(TO_CHAR(CODSG, 'FM0000000'), L_LINE)
        || L_DELIMITER
        || NVL(SOCCODE, L_LINE)
        || L_DELIMITER
        ||
        (SELECT DECODE(RS.RTYPE,'P',DECODE(TO_CHAR(SOCCODE),'SG..',L_LINE,NVL(GET_SIREN(R.SOCCODE), L_LINE)),NVL(GET_SIREN(R.SOCCODE), L_LINE))
        FROM DUAL
        )
        || L_DELIMITER
        ||
        (SELECT DECODE(rs.rtype,'P',DECODE(TO_CHAR(SOCCODE),'SG..',L_LINE,NVL(SUBSTR(SOCLIB,1,10), L_LINE)),NVL(SUBSTR(SOCLIB,1,10), L_LINE))
        FROM societe st
        WHERE st.soccode = r.soccode
        )
        || L_DELIMITER
        || NVL(
        (SELECT CODE_DOMAINE FROM PRESTATION PR WHERE PR.PRESTATION = R.PRESTATION
        ), L_LINE)
        || L_DELIMITER
        || NVL(prestation, L_LINE)
        || L_DELIMITER
        || NVL(MODE_CONTRACTUEL_INDICATIF, L_LINE)
        || L_DELIMITER
        || NVL(TO_CHAR(CPIDENT), L_LINE)
        || L_DELIMITER
        || NVL(TO_CHAR(FIDENT), L_LINE)
        || L_DELIMITER
        || DECODE(rs.rtype,'P',DECODE(TO_CHAR(SOCCODE),'SG..',NVL(NIVEAU, L_LINE),L_LINE),L_LINE)
        || L_DELIMITER
        || DECODE(RS.RTYPE,'P',NVL(TO_CHAR('  '||DISPO), L_LINE),L_LINE)
        || L_DELIMITER
        || DECODE(RS.RTYPE,'P',DECODE(TO_CHAR(SOCCODE),'SG..',L_LINE,NVL(TO_CHAR(COUT,'999990D00'), L_LINE)),NVL(TO_CHAR(COUT,'999990D00'), L_LINE))
        || ' '
        || DECODE(RS.RTYPE,'E','HT','F','HT','L','HTR','P',DECODE(TO_CHAR(SOCCODE),'SG..','HTR','HT') ,L_LINE)
        || L_DELIMITER
        || DECODE(rs.rtype,'E',NVL(TO_CHAR(MONTANT_MENSUEL), L_LINE),'F',NVL(TO_CHAR(MONTANT_MENSUEL), L_LINE),L_LINE)
      FROM situ_ress r ,
        ressource rs
      WHERE r.ident = TO_NUMBER(l_ident)
      AND r.ident   = rs.ident
            ...
            if(p_nomcont is not null) then
                ...
                OPEN    p_curseur FOR
                      select
                          ...
                          RPAD(NVL(TO_CHAR(decode(r.RTYPE,'P',decode((select soccode from situ_ress where ident = r.IDENT and datsitu = (select max(datsitu) from situ_ress where ident = r.IDENT)),'SG..','SG','Presta. au temps passé'),'L','Logiciel','E','Forfait sans frais d''env','F','Forfait avec frais d''env')), '   '), 28, ' ') -- SZ correction ano : si ressource inactive alor type=presta
                          from ressource r
                          where r.rnom like '%'||UPPER(p_nomcont)||'%'
                          and pack_consult_ress.hab_perime(r.ident,l_perime) = 1
