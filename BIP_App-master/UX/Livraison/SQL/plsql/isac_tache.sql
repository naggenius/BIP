...
p_message:='';
  ...
  -- FAD PPM 65123 : Ajout du contr¿le du consomm¿ sur la table PMW_BIPS et CONS_SSTACHE_RES_MOIS
	...
	SELECT cstrm.cusag
	FROM cons_sstache_res_mois cstrm, isac_tache it, isac_etape itp
	WHERE it.tache=to_number(p_tache)
	AND it.pid = cstrm.pid
	AND it.acta = cstrm.acta
	AND it.pid = itp.pid
	AND it.etape = itp.etape
...
BEGIN
  ...
  l_tacheaxemetier := rtrim(p_tacheaxemetier);
