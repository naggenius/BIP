...
BEGIN
    ...
    --QC 1844 - PPM 61163
                ...
                (SELECT d2.CODDIR FROM STRUCT_INFO si,directions d2 WHERE si.CODSG=ligne.CODSG AND  d2.coddir = si.coddir) direction ,

                (SELECT nvl(SUM(nvl(BUD.BPMONTMO,0)),0) FROM BUDGET BUD
                WHERE BUD.PID=ligne.PID  AND BUD.ANNEE=to_number(to_char(L_ANNEE_FONCTIONELLE,'YYYY')))  proposeN,

                (SELECT nvl(SUM(nvl(BUD.BPMONTMO,0)), 0) FROM BUDGET BUD
                WHERE BUD.PID=ligne.PID AND BUD.ANNEE=to_number(to_char(L_ANNEE_FONCTIONELLE,'YYYY'))+1) proposeN1,

                (SELECT nvl(SUM(nvl(BUD.BPMONTMO,0)),0) FROM BUDGET BUD
                WHERE BUD.PID=ligne.PID AND BUD.ANNEE=to_number(to_char(L_ANNEE_FONCTIONELLE,'YYYY'))+2) proposeN2,

                (SELECT nvl(SUM(nvl(BUD.BPMONTMO,0)),0) FROM BUDGET BUD
                WHERE BUD.PID=ligne.PID AND BUD.ANNEE=to_number(to_char(L_ANNEE_FONCTIONELLE,'YYYY'))+3) proposeN3,

                (SELECT nvl(SUM(nvl(BUD.ANMONT,0)),0) FROM BUDGET BUD
                WHERE BUD.PID=ligne.PID AND BUD.ANNEE=to_number(to_char(L_ANNEE_FONCTIONELLE,'YYYY'))) aribtre,

                (SELECT nvl(SUM(nvl(BUD.REESTIME,0)),0) FROM BUDGET BUD
                WHERE BUD.PID=ligne.PID AND BUD.ANNEE=to_number(to_char(L_ANNEE_FONCTIONELLE,'YYYY'))) restime,
    ...
    --QC 1844 - PPM 61163
		...
		(SELECT DISTINCT LB.PID id,
                    ...
                    DECODE(DPC.dpcopiaxemetier, NULL, DECODE(PI1.PROJAXEMETIER, NULL,decode(lb.LINEAXISBUSINESS2,NULL,IT.tacheaxemetier,lb.LINEAXISBUSINESS2), PI1.PROJAXEMETIER), DPC.dpcopiaxemetier) AXE,
                    DECODE(DPC.dpcopiaxemetier, NULL, DECODE(PI1.PROJAXEMETIER, NULL,decode(lb.LINEAXISBUSINESS2,NULL,'T','L'), 'R'), 'D') INDICE
		FROM LIGNE_BIP LB
		...
		WHERE (DPC.dpcopiaxemetier IS NOT NULL OR PI1.PROJAXEMETIER IS NOT NULL OR IT.tacheaxemetier IS NOT NULL OR lb.LINEAXISBUSINESS2 IS NOT NULL)) AXE
    WHERE -- (PACK_DIVA.is_dir_code_client(ligne.clicode)='1' OR PACK_DIVA.is_dir_dpg(ligne.codsg)='1')                 
                 ligne.pid = axe.id
                AND   (ligne.adatestatut IS NULL
