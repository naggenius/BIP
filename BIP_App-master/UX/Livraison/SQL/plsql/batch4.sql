...
-- Begin BIP-409
		...
		update TACHE
			SET cdeb_max= (SELECT ligne_bip.adatestatut FROM ligne_bip WHERE ligne_bip.pid=tache.pid)--'01/01/2010'
				, motif_rejet=REJET_STATUT
			WHERE (aistty!='FF' OR aistty IS NULL) and ((SELECT count(*) FROM ligne_bip WHERE ligne_bip.pid=tache.pid and adatestatut<l_moismens)>0)  ;


    -- BIP-198 Add control for direct BIP line for outsourcing
		l_statement:='Maj de la date pour les motifs REJET_STATUT_SS_TRAITANCE';
		update TACHE
			SET cdeb_max= (select  case when ((l.ADATESTATUT is null) and (j.ADATESTATUT is null)) then null
when ((l.ADATESTATUT is null) and (j.ADATESTATUT is not null)) then j.ADATESTATUT
when ((l.ADATESTATUT is not null) and (j.ADATESTATUT is null)) then l.ADATESTATUT
else least(l.ADATESTATUT,j.ADATESTATUT) end "datestatut" from ligne_bip l,ligne_bip j where l.pid=tache.aistpid and j.pid=tache.PID)--'01/01/2010'
				, motif_rejet=REJET_STATUT_SS_TRAITANCE
			where AISTTY='FF' and ((select count(*) from ligne_bip l,ligne_bip j
    where l.pid=tache.aistpid and j.pid=tache.PID and ((l.ADATESTATUT<l_moismens)or(j.ADATESTATUT <l_moismens) )  )>0);
...
-- END BIP-409      
	...
	--	l_statement:='Maj de la date pour les motifs DATESTATUS less then fontional month';
    
    
	
  -- BIP-198 DELETE this check with TOPFER
  /*	UPDATE tache
