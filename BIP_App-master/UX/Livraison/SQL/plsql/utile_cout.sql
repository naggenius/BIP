...
/*
  BIP-125 changes
  Procedure to find compatable localized profile for monthly processing
*/

PROCEDURE SEARCH_LOCALIZE(
    P_CODSG     IN NUMBER,
    P_CAFI      IN NUMBER,
    P_CDEB      IN VARCHAR2,
    P_CODDIR    IN NUMBER,
    ...
    P_TYPE_PROFIL	OUT NUMBER,
    P_PROFIL_FI OUT VARCHAR2,
    P_COUT_FT   OUT NUMBER,
    P_COUT_ENV  OUT NUMBER)
IS
    L_PROFIL PROFIL_LOCALIZE%ROWTYPE;
	  L_PROFIL_TMP PROFIL_LOCALIZE%ROWTYPE;
    L_CAFI STRUCT_INFO.CAFI%TYPE;

      CURSOR C_PROFIL_LOCALIZE (L_CODDIR IN NUMBER) IS
			SELECT * FROM (
      SELECT DISTINCT P.*
      FROM PROFIL_LOCALIZE P
			INNER JOIN CENTRE_ACTIVITE CA ON (P.CODE_ES IN (CA.CODCAMO,
      CA.CANIV1,
      CA.CANIV2,
      CA.CANIV3,
      CA.CANIV4,
      ...
      CA.CANIV6))
			WHERE P.TOP_ACTIF = 'O'
			AND P.CODDIR = L_CODDIR
			AND P.DATE_EFFET <= TO_DATE(P_CDEB,'dd/MM/yyyy')
			AND P.PROFIL_DEFAUT = 'N'
			AND CA.CDATEFERM IS NULL
      AND NVL(SUBSTR(P_CODSG,-1),'') =  NVL(P.CODE_LOCALIZE,'')
      AND p.CODE_ES <> '*'
      UNION ALL
      SELECT DISTINCT P.*
      FROM PROFIL_LOCALIZE P
      ...
      AND NVL(SUBSTR(P_CODSG,-1),'') =  NVL(P.CODE_LOCALIZE,'')
      AND p.CODE_ES = '*')
			ORDER BY DATE_EFFET DESC, PROFIL_LOCALIZE ASC;
      
      CURSOR C_PROFIL_LOCALIZE_DEFAUT (L_CODDIR IN NUMBER) IS
			SELECT P.*
      FROM PROFIL_LOCALIZE P
			WHERE P.TOP_ACTIF = 'O'
			AND P.CODDIR = L_CODDIR
			AND P.DATE_EFFET <= TO_DATE(P_CDEB,'dd/MM/yyyy')
			AND P.PROFIL_DEFAUT = 'O'
      AND ROWNUM <=1
			ORDER BY P.DATE_EFFET DESC, P.PROFIL_LOCALIZE ASC;

BEGIN

  --check normal case
	OPEN C_PROFIL_LOCALIZE (P_CODDIR);
		LOOP
			FETCH C_PROFIL_LOCALIZE INTO L_PROFIL_TMP;
			EXIT WHEN C_PROFIL_LOCALIZE%NOTFOUND;
      --DBMS_OUTPUT.PUT_LINE('In the scope of normal cursor1 : ' ||L_PROFIL_TMP.CODE_ES||'   '||L_PROFIL_TMP.PROFIL_LOCALIZE);
      IF L_PROFIL_TMP.CODE_ES = '*' THEN
        L_PROFIL := L_PROFIL_TMP;
        EXIT; 
      ELSE
      --finding code cafi of resource --STARTS
      BEGIN
        SELECT CAFI INTO L_CAFI FROM STRUCT_INFO WHERE CODSG = P_CODSG AND CODDIR = P_CODDIR;
      EXCEPTION
      WHEN NO_DATA_FOUND THEN
      L_CAFI := P_CAFI;
      END;  
      --finding code cafi of resource --ENDS
        IF (VERIF_CA(L_PROFIL_TMP.CODE_ES, L_CAFI) = TRUE) THEN
         --DBMS_OUTPUT.PUT_LINE('Checking true CAFI ');
          L_PROFIL := L_PROFIL_TMP;
          EXIT;
           --else
           --DBMS_OUTPUT.PUT_LINE('Checking  CAFI false');
        END IF;
      END IF;
		END LOOP;
	CLOSE C_PROFIL_LOCALIZE;
  
  IF (L_PROFIL.PROFIL_LOCALIZE IS NULL) THEN
      --if normal case is null then check default case
      --DBMS_OUTPUT.PUT_LINE('In the scope of default cursor2');
      OPEN C_PROFIL_LOCALIZE_DEFAUT (P_CODDIR);
      FETCH C_PROFIL_LOCALIZE_DEFAUT INTO L_PROFIL;
      CLOSE C_PROFIL_LOCALIZE_DEFAUT;

    END IF;


    IF (L_PROFIL.PROFIL_LOCALIZE IS NOT NULL) THEN
    --DBMS_OUTPUT.PUT_LINE('In the scope of normal cursor2');
      P_PROFIL_FI := L_PROFIL.PROFIL_LOCALIZE;
      P_COUT_FT := L_PROFIL.FORCE_TRAVAIL;
      P_COUT_ENV := L_PROFIL.FRAIS_ENVIRONNEMENT;
      --LOCALIZE SET
      P_TYPE_PROFIL := 3;
    ELSE
    --DBMS_OUTPUT.PUT_LINE('In the scope NOT EVEN IN DEFAULT');
    --if default case is also null, check any localize irrespective of year
    BEGIN
    SELECT P.* INTO L_PROFIL
      FROM PROFIL_LOCALIZE P
			WHERE P.TOP_ACTIF = 'O'
			AND P.CODDIR = P_CODDIR
			AND P.PROFIL_DEFAUT = 'O'
      AND ROWNUM <=1
			ORDER BY P.DATE_EFFET DESC, P.PROFIL_LOCALIZE ASC;
      EXCEPTION
      ...
      IF L_PROFIL.PROFIL_LOCALIZE IS NOT NULL THEN
      --if default case is also null, check any localize irrespective of year <<EXISTS>>
        P_TYPE_PROFIL := 1;
        P_PROFIL_FI := 'STANDARD';        
      ELSE
      --if default case is also null, check any localize irrespective of year <<NOT EXISTS>>
        P_TYPE_PROFIL := 2;
      END IF;
    END IF;
    
--    DBMS_OUTPUT.PUT_LINE('P_TYPE_PROFIL : ' || P_TYPE_PROFIL);    
--    DBMS_OUTPUT.PUT_LINE('P_PROFIL_FI : ' || P_PROFIL_FI);
--    DBMS_OUTPUT.PUT_LINE('P_COUT_FT : ' || P_COUT_FT);
--    DBMS_OUTPUT.PUT_LINE('P_COUT_ENV : ' || P_COUT_ENV);
...
/*Changes for BIP_125 profile localize check*/
--DBMS_OUTPUT.PUT_LINE('start localize process');
--DBMS_OUTPUT.PUT_LINE('P_CODSG : ' ||P_CODSG);
--DBMS_OUTPUT.PUT_LINE('P_CAFI : ' ||P_CAFI);
--DBMS_OUTPUT.PUT_LINE('P_CDEB : ' ||P_CDEB);
--DBMS_OUTPUT.PUT_LINE('L_CODDIR : ' ||L_CODDIR);
--DBMS_OUTPUT.PUT_LINE('P_CODCAMO : ' ||P_CODCAMO);
--BIP-227/125 CHANGES
  BEGIN
    SELECT CODE_DOMAINE INTO L_CODE_DOMAINE FROM PRESTATION WHERE PRESTATION = P_PREST;
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
    L_CODE_DOMAINE := NULL;
  END;

  IF L_CODE_DOMAINE = 'CO' 
    THEN
        P_TYPE_PROFIL := 1;
        P_PROFIL_FI := 'STANDARD';
    ELSE
       SEARCH_LOCALIZE( P_CODSG, P_CAFI, P_CDEB, L_CODDIR, P_CODCAMO, P_TYPE_PROFIL, P_PROFIL_FI, P_COUT, P_COUT_ENV);    
  END IF;
    
    IF P_TYPE_PROFIL = 2 THEN
        P_PROFIL_FI := 'STANDARD';
...
--if default profil localize case is also null, check any localize irrespective of year <<EXISTS>> then perform direct STANDARD calculation
IF P_TYPE_PROFIL = 1 THEN
  L_STANDARD_FLAG := TRUE;

  CALCUL_COUT_STD ( P_CODSG, P_CAFI, P_PREST, P_IDENT, P_CDEB, P_SOCCODE, P_RTYPE, P_METIER, P_NIVEAU, P_CODCAMO, P_TYPE_PROFIL, 
  P_PROFIL_FI, P_COUT, P_COUT_ENV);
END IF;
  


	IF (p_profil_fi = 'STANDARD' AND L_STANDARD_FLAG = FALSE) THEN
		-- Type Standard ou FI
    		p_type_profil := 1;
...
BEGIN
  ...
  INTO L_COUNT
  FROM
    (SELECT COUTFT+COUTENV AS CNT,
      RANK() OVER (PARTITION BY pid ORDER BY CDEB) RN
    FROM STOCK_FI
    WHERE PID   = P_PID
    AND IDENT   = P_IDENT
...
IF(p_codcamo=77777) THEN
  ...
  where PID=p_pid AND datdeb<=p_cdeb AND datfin IS NULL
		...
		END LOOP;
  /*************BIP-365 Rules enhancement for Dynamic FI Profile calculation******STARTS*********************/
  ELSIF P_PREST IN ('GRA', 'IFO', 'MO', 'INT','STA') OR P_CAFI IN ('0','99999','88888','99810' ) 
    THEN 
    T_PROFIL_FI:='N/A';
  ...
  ELSIF P_CAFI = '99820'
    THEN
          T_PROFIL_FI := P_CAFI;
    /*************BIP-365 Rules enhancement for Dynamic FI Profile calculation*******ENDS*****/
