...
# Nom du fichier : diva_lignes.sh
# Objet : Shell de constitution du fichier des lignes Bip et Notifiés pour DIVA 
...
if [ -z $LOG_TRAITEMENT ]
then
	LOG_FILE=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.diva_lignes.log
...
log_tmp=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.diva_lignestmp.txt

datedebut=`date +'%d/%m/%Y'`
heuredebut=`date +'%T'`

#Répertoire d'extraction : BIPDATA/DIVA/EXPORT_LIGNES
REP_EXTRACT=$PL_EMISSION

logText "--------------------------------------------------------------------------------" >> $LOG_FILE
logText "Debut de DIVA Lignes : diva_lignes.sh" >> $LOG_FILE
...
if [ $PLUS_EXIT -ne 0 ]
then
	logText "Echec de lecture du mois et de l'annee dans diva_lignes.sh" >> $LOG_FILE
...
sqlplus -s $CONNECT_STRING <<! >> $log_tmp 2>&1
	whenever sqlerror exit failure;
	!echo "pack_oscar_lignebip.diva_lignes_notifie"
	exec pack_oscar_lignebip.diva_lignes_notifie('$REP_EXTRACT', '$nom_fic')
!
...
then
	...
	insert into BATCH_LOGS_BIP values ('diva_lignes.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,0, 'NOK');
EOF
	#YNI Ecriture dans le fichier quotidienne.txt dans le cas NOK
	echo " diva_lignes.sh ECHEC" >> $FILE_QUOTIDIENNE
	#Fin YNI
	logText "Probleme SQL*PLUS dans diva_lignes_notifie pour la generation du fichier" >> $LOG_FILE
	logText "Code erreur : <$PLUS_EXIT>" >> $LOG_FILE
	rm -f $log_tmp
	logText "=== ERREUR : Fin ANORMALE de DIVA lignes : diva_lignes.sh ===" >> $LOG_FILE
	exit -1;
else
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
	...
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('diva_lignes.sh','$datedebut', '$heuredebut','$datefin','$heurefin',null,'$nbligne',0, 'OK');
EOF
	#YNI Ecriture dans le fichier quotidienne.txt dans le cas OK
	echo " diva_lignes.sh OK" >> $FILE_QUOTIDIENNE
	#Fin YNI
	rm -f $log_tmp
	logText "=== Fin NORMALE de DIVA lignes : diva_lignes.sh ===" >> $LOG_FILE
fi
