...
EOF
	...
	commit;
EOF
	logText  "Problème script ligne_bip_maj_masse.sh : Appel procédure SQL*PLUS : pack_ligne_bip_maj_masse.maj_masse_ligne_bip "	
	logText "Code erreur : <$PLUS_EXIT>"
	logText  "Problème script ligne_bip_maj_masse.sh : Appel procédure SQL*PLUS : pack_ligne_bip_maj_masse.maj_masse_ligne_bip " >> $LOG_FILE								
	logText "Problème SQL*PLUS lors de l'exécution de la procédure  pack_ligne_bip_maj_masse.maj_masse_ligne_bip : consulter $LOG_FILE"
...
then
  ...
  do
	sqlplus -s $CONNECT_STRING << EOF
	insert into TRAIT_BATCH_RETOUR (ID_TRAIT_BATCH, DATA, ERREUR) VALUES ($id_batch,SUBSTR('$ligne',1,4),'Ligne non traitee, suite format incorrect');
	commit;
EOF
	sqlplus -s $CONNECT_STRING << EOF
