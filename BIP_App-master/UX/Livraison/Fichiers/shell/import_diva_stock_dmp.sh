...
# Nom du fichier : import_diva_stock_dmp.sh
...
# ############
	...
	if [ -z $LOG_TRAITEMENT ]
	then
		LOG_FILE=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.import_diva_stock_dmp.log
...
log_tmp=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.import_diva_stock_dmptmp.txt
statuttmp=0
...
logText "--------------------------------------------------------------------------------" >> $LOG_FILE
logText "Debut de l'import des dmp DIVA : import_diva_stock_dmp.sh" >> $LOG_FILE
...
logText "Fichier detecte, Lancement de import_diva_stock_dmp.sh" >> $LOG_FILE
...
# On supprime les doublons du fichier en entrée
###################################################################################################
...
# ####################################
mv $FILE_IMPORT_DMP  $DIR_BATCH_DATA/$FILE_DIVA_DMP.`date +"%Y%m%d%H%M%S"`
...
then
	...
	insert into BATCH_LOGS_BIP values ('import_diva_stock_dmp.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,'$nbligne', 'NOK');
EOF
	#YNI Ecriture dans le fichier quotidienne.txt dans le cas NOK
	echo " import_diva_stock_dmp.sh ECHEC" >> $FILE_QUOTIDIENNE
	#Fin YNI
	logText "Erreur : consulter $LOG_FILE"
	logText "et le fichier $PL_LOGS/AAAA.MM.JJ.import_diva_stock_dmp.log du jour"
	logText "Erreur de maj des dmp DIVA sous SQL*Plus" >> $LOG_FILE
	logText "Code erreur : <$PLUS_EXIT>" >> $LOG_FILE
	rm -f $log_tmp
	logText "=== ERREUR : Fin ANORMALE du traitement d insertion des donnees dmp en provenance de DIVA : import_diva_stock_dmp.sh. ===" >> $LOG_FILE
	exit -1
else
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
	...
	then
		statut='NOK'
		varlog=`sed "s/$/<br>/; s/'/ /g" $log_tmp`
		logText "=== ERREUR Fin NORMALE du traitement d insertion des donnees dmp en provenance de DIVA : import_diva_stock_dmp.sh. ===" >> $LOG_FILE
		#YNI Ecriture dans le fichier quotidienne.txt dans le cas NOK
		echo " import_diva_stock_dmp.sh ECHEC" >> $FILE_QUOTIDIENNE
		#Fin YNI
	else
		statut='OK'
		logText "=== Fin NORMALE du traitement d insertion des donnees dmp en provenance de DIVA : import_diva_stock_dmp.sh. ===" >> $LOG_FILE
		#YNI Ecriture dans le fichier quotidienne.txt dans le cas OK
		echo " import_diva_stock_dmp.sh OK" >> $FILE_QUOTIDIENNE
		#Fin YNI
	fi
	nom_fichier=$DIR_BATCH_DATA/$FILE_DIVA_DMP
	nbligne=`awk 'END {print NR}' $nom_fichier`
	...
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('import_diva_stock_dmp.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,'$nbligne', '$statut');
EOF
	...
	varlog=`sed "s/$/<br>/; s/'/ /g" $log_tmp`
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('import_diva_stock_dmp.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,0, 'NOK');
EOF
		#YNI Ecriture dans le fichier quotidienne.txt dans le cas OK
		echo " import_diva_stock_dmp.sh ECHEC" >> $FILE_QUOTIDIENNE
		#Fin YNI
