...
EOF
		...
		if [ -e $BAD_FILE ]
		then
			rejectcnt=`cat $BAD_FILE | wc -l`
			echo " PRA_RESS_CONT.sh (contrats/lignes contrat) ECHEC PARTIEL : $rejectcnt rows rejected." >> $FILE_QUOTIDIENNE
...
# -----------------------------------------------------------
	...
	sqlplus -s $CONNECT_STRING << EOF >> $log_tmp 2>&1
		whenever sqlerror exit failure;
		!echo "exec PACK_PRA_RESS_CONT.P_PRA_RESS_CONT_MAIN"
		exec PACK_PRA_RESS_CONT.P_PRA_RESS_CONT_MAIN('$PL_LOGS');
EOF
...
EOF
		...
		echo " PRA_RESS_CONT.sh(PACKAGE PACK_PRA_RESS_CONT.P_PRA_RESS_CONT_MAIN ) ECHEC" >> $FILE_QUOTIDIENNE
