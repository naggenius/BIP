...
# Nom du fichier : 1premensuelle.sh
# Objet : Shell batch (Premiere Premensuelle)
#
#_______________________________________________________________________________
...
# EGR	  15/11/02   run_oscar.sh n'est plus utilise, oscar.sh est directement appele
# NBM     21/11/03   ajout suivi_financier.sh :maj table CUMUL_CONSO pour IAS
# PJO	  03/11/04   Fiche 90 : suppression du shell de reports.sh
...
# DDI     11/05/07   Suppression des traitements OSCAR.  
...
print "Début de la première prémensuelle"

LOG_TRAITEMENT=$DIR_BATCH_SHELL_LOG/$(date +"%Y%m%d").1premensuelle.log
export LOG_TRAITEMENT
logText "`basename $0` : fichier de trace : $LOG_TRAITEMENT"

log_tmp=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.1premensuelletmp.txt
statuttmp=0
...
$DIR_BATCH_SHELL/debfintrait.sh 'DEBUT TRAITEMENT 1 PRE-MENSUELLE' 2>> $LOG_TRAITEMENT
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
	...
	insert into BATCH_LOGS_BIP values ('1premensuelle.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,0, 'NOK');
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'debfintrait.sh'
	#Fin YNI
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'rbip_extract.sh'
	#Fin YNI
...
EOF
    #YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'concatFicBip.sh'
	#Fin YNI
...
################################################################################
logText "-- Lancement de st54700b.sh" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/st54700b.sh 2>> $LOG_TRAITEMENT
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
	varlog="Echec lors du traitement st54700b.sh"
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('1premensuelle.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,0, 'NOK');
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'st54700b.sh'
	#Fin YNI
	logText "Problème dans le traitement st54700b"
	logText "arrêt du batch `basename $0`"
	unset LOG_TRAITEMENT
	exit 110;
fi
################################################################################
logText "-- Lancement de strloapmw.sh" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/stloapmw.sh $FILE_PMW_1 2>> $LOG_TRAITEMENT
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'stloapmw.sh'
	#Fin YNI
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'isac.sh'
	#Fin YNI
...
################################################################################
logText "-- Lancement de alim12.sh" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/alim12.sh 2>> $LOG_TRAITEMENT
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
	varlog="Echec lors du traitement alim12.sh"
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('1premensuelle.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,0, 'NOK');
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'alim12.sh'
	#Fin YNI
	logText "Problème dans le traitement d alimentation de la table suivijhr"
	logText "arrêt du batch `basename $0`"
	unset LOG_TRAITEMENT
	exit 130;
fi
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'st54700e.sh'
	#Fin YNI
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 2 'suivi_financier.sh'
	#Fin YNI
...
################################################################################
logText "-- Lancement de debfintrait.sh" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/debfintrait.sh 'FIN TRAITEMENT 1 PRE-MENSUELLE' 2>> $LOG_TRAITEMENT
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
...
################################################################################
logText "------------------------------------------------------------------------" >> $LOG_TRAITEMENT
logText "Fin de la première prémensuelle" >> $LOG_TRAITEMENT
logText "------------------------------------------------------------------------" >> $LOG_TRAITEMENT
################################################################################
logText "------------------------------------------------------------------------" >> $LOG_TRAITEMENT
...
$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 4 ""
...
then
	...
	logText "=== ERREUR Fin ANORMALE de la premiere prémensuelle ===" >> $LOG_TRAITEMENT
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 3 "$VAR_TRT_ADD"
	#Fin YNI
else
	statut='OK'
	logText "=== Fin NORMALE de la premiere prémensuelle ===" >> $LOG_TRAITEMENT
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 1 $heuredebut 1 "$VAR_TRT_ADD" 
	#Fin YNI
fi
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('1premensuelle.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,0, '$statut');
EOF
	rm -f $log_tmp
