...
# Objet : Shell batch STLOADIMMEUBLE
...
if [ -z $LOG_TRAITEMENT ]
then
	LOG_FILE=$DIR_BATCH_SHELL_LOG/`date +"%Y%m%d"`.stloadimmeuble.log
...
logText "--------------------------------------------------------------------------------" >> $LOG_FILE
logText "Debut de SQLLoader pour les immeubles : stloadimmeuble.sh" >> $LOG_FILE

DATA_FILE=$1
logText $DATA_FILE
#DATA_FILE=$DIR_BATCH_DATA/IMMEUBLE.COL
#LOGLOAD_FILE=$PL_LOGS/`date +"%Y%m%d"`.stloadimmeuble
LOGLOAD_FILE=$DIR_SQLLOADER_LOG/`date +"%Y%m%d"`.stloadimmeuble
...
sqlldr $CONNECT_STRING \
	parfile=$DIR_SQLLOADER_PARAM/stloadimmeuble.par \
	control=$DIR_SQLLOADER_PARAM/stloadimmeuble.ctl \
	data=$DATA_FILE \
	log=$LOGLOAD_FILE \
	bad=$BAD_FILE \
...
then
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
	logText "Problème SQL*LOAD dans batch STLOADIMMEUBLE : consulter $LOG_FILE"
	varlog='ERREUR de chargement des immeubles SQL*Loader'
	nom_fichier=$DATA_FILE
	nbligne=`awk 'END {print NR}' $nom_fichier`
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('stloadimmeuble.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,'$nbligne', 'NOK');
EOF
	#YNI Ecriture dans le fichier quotidienne.txt dans le cas NOK
	echo " stloadimmeuble.sh ECHEC" >> $FILE_QUOTIDIENNE
	#Fin YNI
	...
	logText "=== ERREUR : Fin ANORMALE de SQLLoader pour les immeubles : stloadimmeuble.sh ===" >> $LOG_FILE
	...
	nbligne=`awk 'END {print NR}' $nom_fichier`
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('stloadimmeuble.sh','$datedebut', '$heuredebut','$datefin','$heurefin',null,0,'$nbligne', 'OK');
EOF
	#YNI Ecriture dans le fichier quotidienne.txt dans le cas OK
	echo " stloadimmeuble.sh OK" >> $FILE_QUOTIDIENNE
	#Fin YNI
	logText "=== Fin NORMALE de SQLLoader pour les immeubles : stloadimmeuble.sh ===" >> $LOG_FILE
	
fi

#On deplace le fichier apres traitement
mv $RECEPTION/$FIC_IMMEUBLE $DIR_BATCH_DATA/$FIC_IMMEUBLE
