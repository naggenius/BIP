...
# SEL	  21/03/16   PPM 60612 QC 1846 : ajout du traitement archivage/purge de la table PMW_BIPS à la fin de la mensuelle.
#############################################################################################################
...
$DIR_BATCH_SHELL/debfintrait.sh 'DEBUT TRAITEMENT MENSUELLE'
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
...
######## etape 2.0 : concatenation des fichiers des remonteurs bip
logText "-- Lancement de rbip_extract.sh -------------------------------------------" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/rbip_extract.sh
SHELL_EXIT=$?
logText "$SHELL_EXIT" >> $LOG_TRAITEMENT
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 3 $heuredebut 2 'rbip_extract.sh'
	#Fin YNI
...
######## etape 2.1 : concatenation des fichiers des remonteurs bip
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 3 $heuredebut 2 'concatFicBip.sh'
	#Fin YNI
...
######## etape 2 : chargement donnees PMW ######################################
logText "-- Lancement de stloapmw.sh -------------------------------------------" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/stloapmw.sh $FILE_PMW_3
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
	heurefin=`date +'%T'`
	varlog="Echec lors du traitement stloapmw.sh"
	sqlplus -s $CONNECT_STRING << EOF
	insert into BATCH_LOGS_BIP values ('mensuelle.sh','$datedebut', '$heuredebut','$datefin','$heurefin','$varlog',0,0, 'NOK');
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 3 $heuredebut 2 'stloapmw.sh'
	#Fin YNI
	logText "Echec lors de l execution du traitement stloadpmw"
	logText "arrêt du batch `basename $0`"
	unset LOG_TRAITEMENT
	exit 310;
fi

######### etape 3 : import des donnees de ISAC #################################
logText "-- Lancement de isac.sh -----------------------------------------------" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/isac.sh 3
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	datefin=`date +'%d/%m/%Y'`
...
EOF
	#YNI Appel au shell alerte_mail.sh
	$DIR_BATCH_SHELL/alerte_mail.sh 3 $heuredebut 2 'isac.sh'
	#Fin YNI
	logText "Echec lors de l execution du traitement isac"
	logText "arrêt du batch `basename $0`"
	unset LOG_TRAITEMENT
	exit 311;
fi
...
EOF
        #YNI Appel au shell alerte_mail.sh
        #BIP 183 - Mailing the information about absent of niveau file
		$DIR_BATCH_SHELL/alerte_mail.sh 4 $heuredebut 2 'import_niveau.sh'
		#Fin YNI
...
############################ SEL PPM 60612 QC 1846 : ajout du traitement de l'archivage/purge de la table PMW_BIPS ################################################################
logText "-- Lancement du traitement archive_purge_pmw_bips" >> $LOG_TRAITEMENT
$DIR_BATCH_SHELL/archive_purge_pmw_bips.sh 2>> $LOG_TRAITEMENT
SHELL_EXIT=$?
if [ $SHELL_EXIT -ne 0 ]
then
	statuttmp=1
	echo "Problème dans archive_purge_pmw_bips" >> $log_tmp   
        logText "Probl\350me dans le traitement archive_purge_pmw_bips.sh"
	VAR_TRT_ADD = "${VAR_TRT_ADD}\n archive_purge_pmw_bips.sh"
fi
