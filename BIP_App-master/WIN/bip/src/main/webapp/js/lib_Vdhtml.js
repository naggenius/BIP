var min_x_size = (varLargeurBandeauVertical<350)?(750-varLargeurBandeauVertical):400;
var min_y_size = 410;
...
function yg_verif_navigateur(){
	...
	this.bw=(this.ie5 || this.ie4 ||this.ie6 || this.ns4 || this.ns5 ||this.ie7 )
	return this
}
...
function yg_verif_pourcent(num,w){
	if(num){
		if(num.toString().indexOf("%")!=-1){
			if(w) {
				num=((page.x2-varLargeurBandeauVertical)*parseFloat(num)/100);
...
function yg_objet(obj,nest){
	...
	function yg_ajuster_contenu(x, y, t, r, b, l, setwidth) { //Ajuste la position et la taille de l'objet
		if (x!=0 || y!=0) {
			this.deplacer(x, y);
			}
		this.redimensionner(t,r,b,l,setwidth);
	}
	//alert(nest + " : " +  document[nest]);
	if(!bw.bw) return message('Old browser')
	this.evnt=bw.dom && document.getElementById(obj)||bw.ie4 && document.all[obj]|| (nest?bw.ns4 && document[nest] && document[nest].document[obj]:bw.ns4 && document.layers[obj]);
...
//	alert("cl="+this.cl);
	...
	this.ajuster = yg_ajuster_contenu;
