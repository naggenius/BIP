/*
// FAD PPM 60955 : Duplication de jqueryMultiSelect.js
// jQuery multiSelect
//
// Version 1.0 beta
//
	...
	$.extend($.fn, {
		...
		var b=false;
			...
			$(this).each( function() {
				...
				html += '<div class="multiSelectOptions" style="z-index: 99999; display: none; font-size : 11px; width : 100%;">';
				...
				if( o.selectAll ) {
					...
					if( sa )
						$(select).next('.multiSelect').next('.multiSelectOptions').find('INPUT.selectAll').attr('checked', true).parent().addClass('checked');
				}
		...
		multiSelectOptionsShow: function() {
			...
			var leftValue = offset.top;
			
			if ($.browser.msie) {
				// Pour IE
				leftValue = $(this)[0].offsetLeft + $(this).width();
			}
			
			$(this).next('.multiSelectOptions').css({ top:  topValue + $(this).outerHeight() + 'px' });
			$(this).next('.multiSelectOptions').css({ left: leftValue + 'px' });
		...
		$(this).prev('INPUT.multiSelect').css ("font-style","italic");
			var i = 0, s = '';
			...
			if(i==1)
			{
			$(this).prev('INPUT.multiSelect').css ("font-style","normal");
			}
		...
		deSelectAll: function(o) {
			var pid_all='';
			$('INPUT:checkbox').attr('checked', false).parent().removeClass('checked');
			$('INPUT.multiSelect').val(o.noneSelected);
			$('INPUT[name="p_param26"]').val(pid_all);
		},
		...
		listerPid: function() {
		    ...
		    //var pid = $('.multiSelect').val().split('-')[0].trim();
			var pid='';
			$('INPUT:checkbox:checked').each( function() {
				//pid += "|" + $(this).val()+ "|" + ',';
				pid += $(this).val() + ',';
			})
			$('INPUT[name="p_param26"]').val(pid);
