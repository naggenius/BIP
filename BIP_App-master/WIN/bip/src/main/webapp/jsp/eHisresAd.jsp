...
<bip:VerifUser page="jsp/eHisresAd.jsp"/>
...
function MessageInitial()
{
	...
	tabVerif["p_param8"]  = "VerifierNum(document.editionForm.p_param8, 5, 0)";
...
function ValiderEcran(form)
{
	if (blnVerification)
	{
		...
		{
			// alert("Les champs 'Code DPG' et 'Code ligne BIP' ne peuvent être saisis simultanément");
			alert("Une seule sélection possible");
			return false;
		}
		if ( (form.p_param7.value.length == 0) && (form.p_param8.value.length == 0) )
		{
			alert("Sélection obligatoire (DPG ou ressource)");
			return false;
		}
...
function rechercheID(){
	window.open("/recupIdPersonne.do?action=initialiser&nomChampDestinataire=p_param8&windowTitle=Recherche Identifiant Personne&habilitationPage=HabilitationLigneBip"  ,"", "toolbar=no, directories=no, location=no, status=no, menubar=no, resizable=yes, scrollbars=yes, width=450, height=450") ;
	return ;
}  
...
<input type="hidden" name="pageAide" value="<%= sPageAide %>">
					...
					<tr>
						<td class="lib">Code ressource :</td>
						<td><html:text property="p_param8" styleClass="input"  size="5" maxlength="5" onchange="return VerifFormat(this.name);"/>
