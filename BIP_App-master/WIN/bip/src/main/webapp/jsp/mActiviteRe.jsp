...
function ValiderEcran(form)
{
  if (blnVerification == true) {
     ...
     if (form.mode.value== 'delete') {
        if (!confirm("Voulez-vous supprimer cette activité ?")) return false;
     }
     else
     {
	 	if (!ChampObligatoire(form.lib_activite, "le libellé de l'activité")) return false;
	
        if (form.mode.value== 'update') {
     		if (!confirm("Voulez-vous modifier cette activité ?")) return false;
     	}
     	else
     	{
     		if (!ChampObligatoire(form.code_activite, "le code activité")) return false;
     	}
     }
