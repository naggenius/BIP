...
<jsp:useBean id="RecupCodeDosProjForm" scope="request" class="com.socgen.bip.form.RecupCodeDosProjForm" />
<jsp:useBean id="listeRechercheDosProj" scope="session"	class="com.socgen.ich.ihm.menu.PaginationVector" />
...
{
   var Message="<bean:write filter="false"  name="RecupCodeDosProjForm"  property="msgErreur" />";
   var Focus = "<bean:write name="RecupCodeDosProjForm"  property="focus" />";
...
function fill(num)
{
	var cible = "<%=RecupCodeDosProjForm.nomChampDestinataire%>";
	var action = "<%=RecupCodeDosProjForm.rafraichir%>";
  ...
  <tr> 
    <td> 
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr> 
          <td height="20" class="TitrePage"><bean:write name="RecupCodeDosProjForm" property="windowTitle" /></td>
        </tr>
        <tr> 
          <td background="../images/ligne.gif"></td>
        </tr>
        ...
        <tr> 
          <td><html:form action="/recupCodeDosProj.do">
...
<html:hidden property="arborescence" value="<%= arborescence %>"/>
                     ...
                     <html:hidden name="RecupCodeDosProjForm" property="nomChampDestinataire"/>
                     <html:hidden name="RecupCodeDosProjForm" property="windowTitle"/>
                     <html:hidden name="RecupCodeDosProjForm" property="habilitationPage"/>  
                                              
                     <input type="hidden" name="index" value="modifier">                                                          
                        <table align="center">					
				        	<tr>
				            	<td class="lib">Code dossier projet :&nbsp;<BR>
         				...
         				<table width=80%>            	
             				<% if (listeRechercheDosProj.size() == 99){%>
            				 <tr>
							...
							<logic:present name="listeRechercheDosProj">   
							<% if  	(listeRechercheDosProj.size()>0){%>
								<tr>
									<td colspan ="4" class="contenu">Cliquez sur un code dossier projet pour le sélectionner</td>
								</tr>
								<BR>
 								<tr>
					 				<td class="lib" width="25%" >Code dossier projet</td>
 									<td class="lib" colspan="3" >Libellé</td>
								</tr>
								<logic:iterate id="element"  name="listeRechercheDosProj"  length="<%=listeRechercheDosProj.getCountInBlock()%>" 
          							offset="<%=listeRechercheDosProj.getOffset(0)%>"
									type="com.socgen.bip.metier.InfosDossierProjet"
									indexId="index"> 
									<% if ( i == 0) i = 1; else i = 0;%>
								...
								<tr>
									<td align="center" colspan="4" class="contenu">
										<bip:pagination beanName="listeRechercheDosProj"/>
									</td>
								</tr>						
							<%}%>				
							</logic:present>                                           	                   
