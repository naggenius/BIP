...
<bip:VerifUser page="jsp/xSuiviMensuel.jsp"/> 
...
function ValiderEcran(form) {
	return Ctrl_dpg_generique(document.extractionForm.p_param6);
	
	// Si report par DPG et chef de projet choisi
	if (form.choixReports[1].checked==true) {
		form.jobId.value="xSuiviMensuelG2";	
	}
...
<html:hidden property="action" value="refresh"/>
			...
			<table border=0  cellpadding=2 cellspacing=2 class="tableBleu">
                ...
                <tr> 
                  <td class="lib">Code DPG : &nbsp </td>
                  <td> 
					<html:text property="p_param6" styleClass="input"  size="7" maxlength="7" onchange="return VerifFormat(this.name);"/>
	...
	<%
		if ("xSuiviMensuel".equals(sJobId)) {		          
    %>            
 		    	<input type="hidden" name="choixReports" value="1">
 		   	 	<input type="hidden" name="choixReports" value="2">      
    <%
		}
		else {			
    %>            
                <tr> 
                  	<td colspan=2 class="lib"><input type=radio name="choixReports" value="1" checked>Par Client et Application</td>
                </tr>
                <tr> 
					<td colspan=2 class="lib"><input type=radio name="choixReports" value="2">Par DPG et Chef de Projet</td>
                </tr>
     <%
		}
	 %>                 
