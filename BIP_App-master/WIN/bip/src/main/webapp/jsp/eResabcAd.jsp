...
<bip:VerifUser page="jsp/eResabcAd.jsp"/>
...
function MessageInitial()
{
	...
	if (Message != "") {
		alert(Message);
		document.editionForm.p_param6.value=replaceString("§",document.editionForm.p_param6.value,"'");	   
	}
...
function replaceString(de,texte,par) 
{ 	
	return eval('texte.replace(/'+de+'/g,par)');
}
...
function ValiderEcran(form)
{
	if (blnVerification)
	{
		...
		if (!ChampObligatoire(form.p_param6, "un nom de ressource")) return false;
	    form.p_param6.value=replaceString("'",form.p_param6.value,"§");	   
	}
...
<input type="hidden" name="jobId" value="<%=sJobId%>">
            ...
            <!-- p_param7 -> liste des centre de frais -->
            <input type="hidden" name="p_param7" value="<%=liste_Centres_Frais%>">
			<!-- #EndEditable -->
...
<input type="hidden" name="pageAide" value="<%= sPageAide %>">
					...
					<tr>
                        <td colspan=2 class="lib"><B>Nom ou premi&egrave;res lettres du nom :</B></td>
                      </tr>
					<tr>
						<td colspan=2 align="center"><html:text property="p_param6" styleClass="input"  size="50" maxlength="50" onchange="return VerifFormat(this.name);"/></td>
					</tr>
					<tr><td colspan=5 align="center">&nbsp;</td></tr><tr>
					<!-- #EndEditable -->
			   		</table>
