...
import com.socgen.bip.form.RecupIdFourCopiForm;
import com.socgen.bip.metier.InfosFourCopi;
import com.socgen.cap.fwk.exception.BaseException;
import com.socgen.ich.ihm.menu.PaginationVector;

public class RecupIdFourCopiAction extends AutomateAction {

	private static String PACK_SELECT = "recup.id.fourcopi.proc";
	...
	protected ActionForward initialiser(ActionMapping mapping, ActionForm form,
		...
		RecupIdFourCopiForm rechercheForm = (RecupIdFourCopiForm) form;
		// Détruit la liste de la session si elle existait suite à uen recherche
		if (request.getSession(false).getAttribute(LISTE_RECHERCHE_ID_FOURCOPI) != null)
			request.getSession(false).removeAttribute(LISTE_RECHERCHE_ID_FOURCOPI);
		try {
			rechercheForm.setAction("modifier");
			rechercheForm.setListePourPagination(null);
			request.setAttribute("RecupIdFourCopiForm", rechercheForm);
		} catch (Exception ex) {
		...
		try {
				...
				if (paramOut.getNom().equals("curseur")) {
					...
					try {
						...
						while (rset.next() && compteur < 99) {
							...
							vListe.add(new InfosFourCopi(rset.getString(1), rset
									.getString(2),
									buildUrl(hrefId)));
							compteur++;
						}
						...
						PaginationVector vueListe = new PaginationVector(
								vListe, linesPerPage);
						request.setAttribute(LISTE_RECHERCHE_ID_FOURCOPI, vueListe);
						(request.getSession(false)).setAttribute(
								LISTE_RECHERCHE_ID_FOURCOPI, vueListe);
						rechercheForm.setListePourPagination(vueListe);
						
					}// try
					catch (SQLException sqle) {
						if (request.getSession(false).getAttribute(
								LISTE_RECHERCHE_ID_FOURCOPI) != null)
							request.getSession(false).removeAttribute(
									LISTE_RECHERCHE_ID_FOURCOPI);
						logBipUser.exit(signatureMethode);
			...
			if (request.getSession(false).getAttribute(LISTE_RECHERCHE_ID_FOURCOPI) != null)
				request.getSession(false)
						.removeAttribute(LISTE_RECHERCHE_ID_FOURCOPI);
			logBipUser.exit(signatureMethode);
			 jdbc.closeJDBC(); return mapping.findForward(processBaseExceptionRecup(
					"RecupIdFourCopiForm", this.getClass().getName(), "consulter",
	...
	public String buildUrl(String code) {
		 ...
		 return ("<a href=\"javascript:fill('" + code + "');\">");
