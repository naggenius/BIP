 ...
 * 
 * Action mise à jour des situations d'un logiciel chemin :
 * Ressources/MAJ/Logicielss/Situation pages : fmSitulogAd.jsp et mSitulogAd.jsp
 * pl/sql : situlog.sql
 */
public class SitulogAction extends LogicielAction {

	private static String PACK_SELECT = "situlog.consulter.proc";

	private static String PACK_INSERT = "situlog.creer.proc";

	private static String PACK_UPDATE = "situlog.modifier.proc";

	private static String PACK_DELETE = "situlog.supprimer.proc";
	
	private static String PACK_RECUP_SOCIETE="societe.recup.soccode";
	
	private static String PACK_RECUP_MODECONT="situlog.recup.modecont.proc";
	...
	/**
	 * Action qui permet de créer un code Situlog
	 */
	protected ActionForward creer(ActionMapping mapping, ActionForm form,
		...
		String signatureMethode = "SitulogAction -creer( mapping, form , request,  response,  errors )";
		...
		try {
			...
			bipForm.setDatsitu("");
			bipForm.setCoulog(((ParametreProc) vParamOut.get(5)).getValeur()
					.toString());
			if (((ParametreProc) vParamOut.get(6)).getValeur() != null) {
				bipForm.setPrestation(((ParametreProc) vParamOut.get(6))
						.getValeur().toString());
			}
			...
			logBipUser.debug("SitulogAction-creer() --> BaseException :" + be);
			logBipUser.debug("SitulogAction-creer() --> Exception :"
					+ be.getInitialException().getMessage());
			logService.debug("SitulogAction-creer() --> BaseException :" + be);
			logService.debug("SitulogAction-creer() --> Exception :"
					+ be.getInitialException().getMessage());
	...
	protected ActionForward consulter(ActionMapping mapping, ActionForm form,
		...
		String signatureMethode = " SitulogAction-consulter(paramProc, mapping, form , request,  response,  errors )";
		...
		try {
			...
			for (Enumeration e = vParamOut.elements(); e.hasMoreElements();) {
				...
				if (paramOut.getNom().equals("modeContractuelInd")) {
					System.out.println("Existance du mode contractuel");
					System.out.println("Valeur du paramout :"+paramOut.getValeur());
					//bipForm.setModeContractuelInd(paramOut.getValeur().toString());
				...
				if (paramOut.getNom().equals("curseur")) {
					...
					try {
						...
						if (rset.next()) {
							...
							bipForm.setOldatsitu(rset.getString(11));
							bipForm.setCoulog(rset.getString(12));
							bipForm.setCode_domaine(rset.getString(13));
						...
						logService
								.debug("SitulogAction-consulter() --> SQLException :"
										+ sqle);
						logBipUser
								.debug("SitulogAction-consulter() --> SQLException :"
										+ sqle);
						...
						try {
							...
							logBipUser
									.debug("SitulogAction-consulter() --> SQLException-rset.close() :"
											+ sqle);
							// Erreur de lecture du resultSet
			...
			if (msg) {
				// le code Situlog n'existe pas, on récupère le message
			...
			logBipUser.debug("SitulogAction-consulter() --> BaseException :"
					+ be, be);
			logBipUser.debug("SitulogAction-consulter() --> Exception :"
					+ be.getInitialException().getMessage());

			logService.debug("SitulogAction-consulter() --> BaseException :"
					+ be, be);
			logService.debug("SitulogAction-consulter() --> Exception :"
					+ be.getInitialException().getMessage());
			// Si exception sql alors extraire le message d'erreur du message
			// global
			if (be.getInitialException().getClass().getName().equals(
