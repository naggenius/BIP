...
import com.socgen.bip.form.RecupPrestationForm;
import com.socgen.bip.metier.InfosPrestation;
import com.socgen.cap.fwk.exception.BaseException;
import com.socgen.ich.ihm.menu.PaginationVector;

/**
 * @author X054232 - JMA - 03/10/2005 Action permettant de récupérer un code
 *         Prestation via une fenêtre Popup
 */

public class RecupPrestationAction extends AutomateAction {
	private static String PACK_SELECT = "recup.id.prestation.proc";
	...
	protected ActionForward initialiser(ActionMapping mapping, ActionForm form,
		...
		RecupPrestationForm rechercheForm = (RecupPrestationForm) form;
		...
		try {
			...
			request.setAttribute("RecupPrestationForm", rechercheForm);
		} catch (Exception ex) {
	...
	/**
	 * Méthode permettant de lister l'ensemble des codes Prestation commençant
	 * par le texte saisi
	 */
		...
		try {
				...
				if (paramOut.getNom().equals("curseur")) {
					...
					try {
						...
						while (rset.next() && compteur < 99) {
							...
							// vector
							vListe.add(new InfosPrestation(rset.getString(1),
									rset.getString(2), buildUrl(rset
											.getString(1))));
							compteur++;
						}
						if (rset != null)
			...
			logBipUser.exit(signatureMethode);
			 jdbc.closeJDBC(); return mapping.findForward(processBaseExceptionRecup(
					"RecupPrestationForm", this.getClass().getName(),
	...
	public String buildUrl(String idPrestation) {
		  return ("<a href=\"javascript:fill('" + idPrestation + "');\">");
	}
