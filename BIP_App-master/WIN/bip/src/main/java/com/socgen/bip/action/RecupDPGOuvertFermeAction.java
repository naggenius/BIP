...
import com.socgen.bip.form.RecupDPGOuvertFermeForm;
...
public class RecupDPGOuvertFermeAction extends AutomateAction {
	private static String PACK_SELECT = "recup.id.dpgouvfer.proc";
	...
	/**
	 * Constructor for RecupDPGOuvertFermeAction.
	 */
	public RecupDPGOuvertFermeAction() {
		super();
	}
	...
	protected ActionForward initialiser(ActionMapping mapping, ActionForm form,
		...
		RecupDPGOuvertFermeForm rechercheForm = (RecupDPGOuvertFermeForm) form;
		...
		try {
			...
			request.setAttribute("RecupDPGOuvertFermeForm", rechercheForm);
		} catch (Exception ex) {
		...
		try {
			...
			logBipUser.exit(signatureMethode);
			 jdbc.closeJDBC(); return mapping.findForward(processBaseExceptionRecup(
					"RecupDPGOuvertFermeForm", this.getClass().getName(), "consulter", be,
					rechercheForm, request));
		}
		logBipUser.exit(signatureMethode);
		request.setAttribute("RecupDPGOuvertFermeForm", rechercheForm);
