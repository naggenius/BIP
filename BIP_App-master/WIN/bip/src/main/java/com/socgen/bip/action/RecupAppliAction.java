...
import com.socgen.bip.metier.InfosAppli;
import com.socgen.cap.fwk.exception.BaseException;
import com.socgen.ich.ihm.menu.PaginationVector;

/**
 * @author X054232 - JMA - 07/10/2005 Action permettant de récupérer un code
 *         Application via une fenêtre Popup
 */

public class RecupAppliAction extends AutomateAction {
	private static String PACK_SELECT = "recup.id.appli.proc";
	...
	protected ActionForward initialiser(ActionMapping mapping, ActionForm form,
		...
		RecupAppliForm rechercheForm = (RecupAppliForm) form;
		...
		try {
			...
			request.setAttribute("RecupAppliForm", rechercheForm);
		} catch (Exception ex) {
	...
	/**
	 * Méthode permettant de lister l'ensemble des codes DPG commençant par le
	 * texte saisi
	 */
		...
		try {
				...
				if (paramOut.getNom().equals("curseur")) {
					...
					try {
						while (rset.next() && compteur < 99) {
							...
							// un vector
							vListe
									.add(new InfosAppli(rset.getString(1), rset
											.getString(2), buildUrl(rset
											.getString(1))));
							compteur++;
						}
						if (rset != null)
			...
			logBipUser.exit(signatureMethode);
			 jdbc.closeJDBC(); return mapping.findForward(processBaseExceptionRecup(
					"RecupAppliForm", this.getClass().getName(), "consulter",
	...
	public String buildUrl(String idAppli) {
		return ("<a href=\"javascript:fill('" + idAppli + "');\">");
	}
