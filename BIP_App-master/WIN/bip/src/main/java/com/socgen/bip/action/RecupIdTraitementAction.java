...
import com.socgen.bip.form.RecupIdTraitementForm;
import com.socgen.bip.metier.TraitBatch;
import com.socgen.cap.fwk.exception.BaseException;
import com.socgen.ich.ihm.menu.PaginationVector;

public class RecupIdTraitementAction extends AutomateAction {

	private static String PACK_SELECT = "recup.id.traitement.proc";

	/**
	 * Constructor for RecupIdPersonneAction.
	 */
	public RecupIdTraitementAction() {
		super();
	}
	...
	protected ActionForward initialiser(ActionMapping mapping, ActionForm form,
		...
		RecupIdTraitementForm rechercheForm = (RecupIdTraitementForm) form;
		// Détruit la liste de la session si elle existait suite à une recherche
		if (request.getSession(false).getAttribute(LISTE_RECHERCHE_ID_TRAITEMENT) != null)
			request.getSession(false).removeAttribute(
					LISTE_RECHERCHE_ID_TRAITEMENT);
		try {
			rechercheForm.setAction("modifier");
			rechercheForm.setListePourPagination(null);
			request.setAttribute("RecupIdTraitementForm", rechercheForm);
	...
	/**
	 * Méthode permettant de lister l'ensemble des traitements
		...
		try {
				...
				if (paramOut.getNom().equals("curseur")) {
					...
					try {
						...
						while (rset.next() && compteur < 99) {
							...
							vListe.add(new TraitBatch(null,rset.getString(2), rset
									.getString(3), String.valueOf(rset.getInt(1)),null,
									buildUrl(hrefId),null,null,null ));
							compteur++;
						}
						if (rset != null)
							rset.close();
						PaginationVector vueListe = new PaginationVector(
								vListe, linesPerPage);
						request.setAttribute(LISTE_RECHERCHE_ID_TRAITEMENT,
								vueListe);
						(request.getSession(false)).setAttribute(
								LISTE_RECHERCHE_ID_TRAITEMENT, vueListe);
						rechercheForm.setListePourPagination(vueListe);
					}// try
					catch (SQLException sqle) {
						if (request.getSession(false).getAttribute(
								LISTE_RECHERCHE_ID_TRAITEMENT) != null)
							request.getSession(false).removeAttribute(
									LISTE_RECHERCHE_ID_TRAITEMENT);
						logBipUser.exit(signatureMethode);
			...
			logBipUser.exit(signatureMethode);
			 jdbc.closeJDBC(); return mapping.findForward(processBaseExceptionRecup(
					"RecupIdTraitementForm", this.getClass().getName(),
	...
	public String buildUrl(String idTraitement) {
		 return ("<a href=\"javascript:fill('" + idTraitement + "');\">");
	}
