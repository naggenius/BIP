...
import com.socgen.bip.form.RecupCodcamoForm;
import com.socgen.bip.metier.InfosCodcamo;
import com.socgen.cap.fwk.exception.BaseException;
import com.socgen.ich.ihm.menu.PaginationVector;

/**
 * @author DDI - 06/09/2005 Action permettant de récupérer un code Codcamo via une
 *         fenêtre Popup
 */

public class RecupCodcamoAction extends AutomateAction {
	private static String PACK_SELECT = "recup.id.codcamo.proc";
	...
	/**
	 * Constructor for RecupCodcamoAction.
	 */
	public RecupCodcamoAction() {
		super();
	}
	...
	protected ActionForward initialiser(ActionMapping mapping, ActionForm form,
		...
		RecupCodcamoForm rechercheForm = (RecupCodcamoForm) form;
		...
		try {
			...
			request.setAttribute("RecupCodcamoForm", rechercheForm);
		} catch (Exception ex) {
	...
	/**
	 * Méthode permettant de lister l'ensemble des codes Codcamo commençant par le
	 * texte saisi
	 */
		...
		try {
				...
				if (paramOut.getNom().equals("curseur")) {
					...
					try {
						...
						while (rset.next() && compteur < 99) {
							...
							vListe.add(new InfosCodcamo(rset.getString(1), rset
			...
			logBipUser.exit(signatureMethode);
			 jdbc.closeJDBC(); return mapping.findForward(processBaseExceptionRecup(
					"RecupCodcamoForm", this.getClass().getName(), "consulter", be,
					rechercheForm, request));
		}
		logBipUser.exit(signatureMethode);
		request.setAttribute("RecupCodcamoForm", rechercheForm);
	...
	public String buildUrl(int idCodcamo) {
		 return ("<a href=\"javascript:fill('" + idCodcamo + "');\">");
	}
