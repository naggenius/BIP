 ...
 */
public class AffectationIdentAction extends AutomateAction {
	...
	protected ActionForward suite(
		...
		if (request.getParameter("mode").equals("consulter")) {
			...
			bipForm.setCodsg(request.getParameter("codsg"));
			bipForm.setIdent(request.getParameter("ident"));
			bipForm.setRessource(request.getParameter("ressource"));
	...
	/**
		* Action qui permet de passer à la page première page avec reconstitution du menu
		*/
	
	protected  ActionForward annuler(
				ActionMapping mapping,
				ActionForm form,
				HttpServletRequest request,
				HttpServletResponse response,
	...
	*/
	...
	protected ActionForward valider(
		...
		logBipUser.exit(signatureMethode);
		 ...
		 jdbc.closeJDBC(); return mapping.findForward("consoTache");
